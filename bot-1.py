...
async def analyze_async(symbol):
    try:
        ...
        # Done here (not in ai_validator) to avoid circular imports
...
async def cmd_scalp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        actionable = [r for r in scalp_results if r and r.get("direction") != "NEUTRAL" and r.get("confidence") in ("HIGH","MEDIUM")]
        if not actionable:
