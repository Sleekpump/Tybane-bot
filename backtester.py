...
MAX_HOLD_BARS_SCALP = 4    # scalp max hold: 4 x 1H bars = 4 hours
RESULTS_FILE   = "backtest_results.json"
SLIPPAGE_PCT   = 0.05  # 0.05% slippage on entry/exit
TAKER_FEE_PCT  = 0.06  # 0.06% taker fee per side (Bitget)
FUNDING_RATE   = 0.01  # 0.01% funding fee per 8h period (Bitget default)
FUNDING_INTERVAL_BARS = 8  # funding charged every 8 x 1H bars
...
def generate_signals_walkforward(
    ...
    df_4h: pd.DataFrame,
    warmup_bars: int = 210,
    step: int = 1,         # check every bar — matches live bot scan frequency
) -> list[BacktestTrade]:
    """
    ...
    for i in range(warmup_bars, n, step):
        ...
        # Signal type classification — mirrors live bot logic exactly
        # Uses group scores from Phase 1 to determine MOMENTUM vs REVERSAL
        gs           = quality_result.get("group_scores", {})
        struct_s     = gs.get("structure", 0)
        mom_s        = gs.get("momentum", 0)
        volume_s     = gs.get("volume", 0)
        confluence   = quality_result.get("confluence_groups", 0)

        # MOMENTUM: structure breakout + momentum + volume confirmation
        # Mirrors classify_signal() thresholds from bot.py
        if direction == "LONG":
            breakout_pts = sum([
                struct_s > 0.3,   # structure agrees bullish
                mom_s > 0.2,      # momentum agrees bullish
                volume_s > 0.2,   # volume confirms
                confluence >= 4,  # strong confluence
            ])
        else:
            breakout_pts = sum([
                struct_s < -0.3,
        ...
        signal_type = "MOMENTUM" if breakout_pts >= 2 else "REVERSAL"
...
def simulate_trades(
    ...
    for trade in trades:
        ...
        # Scalp trades expire after 4H, swing after 72H
        hold_max = MAX_HOLD_BARS_SCALP if trade.signal_type == "SCALP" else MAX_HOLD_BARS
        end      = min(start + hold_max, len(df_1h))
        ...
        # PnL calculation
        # Correct fee: 2 sides (entry + exit) at taker rate on leveraged position
        # fee_pct = 2 * TAKER_FEE_PCT * LEVERAGE (e.g. 2 * 0.06% * 10 = 1.2% of risked capital)
        ...
        fee_pct       = 2 * TAKER_FEE_PCT * LEVERAGE  # both entry and exit fees

        # Funding rate cost — charged every 8 hours on open position
        # Longs pay funding when rate is positive (most of the time)
        # Cost = (bars_held / 8) * FUNDING_RATE * LEVERAGE
        funding_periods = max(0, trade.bars_held // FUNDING_INTERVAL_BARS)
        funding_pct     = funding_periods * FUNDING_RATE * LEVERAGE

        net_pct       = leveraged_pct - fee_pct - funding_pct

        trade.pnl_pct   = round(net_pct, 2)
        trade.pnl_usdt  = round(risk_per_trade * net_pct / 100, 2)

        # Reclassify expired trades by actual PnL
        if trade.outcome == "EXPIRED":
            if trade.pnl_usdt > 0:
                trade.outcome = "WIN"       # expired in profit = win
            elif trade.pnl_usdt < 0:
                trade.outcome = "LOSS"      # expired in loss = loss
            else:
                trade.outcome = "BREAKEVEN" # exactly flat
...
def format_backtest_report(results: dict) -> list[str]:
    ...
    _tpnl = overall.get('total_pnl_usdt', 0)
    msg1 += "  Total PnL: `" + ("+" if _tpnl >= 0 else "") + "{:.2f}".format(_tpnl) + " USDT`\n"
    ...
    msg1 += "\n*By Market Regime:*\n"
    regime_emojis = {"TRENDING_UP": "📈", "TRENDING_DOWN": "📉", "RANGING": "\u2194"}
    ...
    for symbol, stats in sorted_coins[:15]:
        ...
        if total > 0:
            ...
            msg2 += ce + " *" + label + "*: WR `" + str(wr_c) + "%` | `" + str(total) + "` trades | PnL `" + pnl_str + " USDT` | PF `" + str(pf) + "` | DD `" + "{:.1f}".format(dd) + "%`\n"
...
async def cmd_backtest(update, ctx, exchange_ref, coin_labels: dict):
    ...
    try:
        ...
        try:
            ...
            log.warning(f"paper_trades.json corrupted: {_jde}")
            trades = []
        ...
        for msg in messages:
            if msg.strip():
                try:
                    ...
                    # Markdown parse error — strip formatting and resend as plain text
                    plain = msg.replace("*","").replace("`","").replace("_","")
                    await update.message.reply_text(plain, parse_mode=None)
    except Exception as e:
        await update.message.reply_text("Backtest error: " + str(e))
