...
No pandas-ta dependency - uses pandas built-in functions only
"""
...
def score_timeframe(df):
    ...
    # MA50 / MA200 using pandas
    ma50 = close.rolling(50).mean()
    ma200 = close.rolling(200).mean()
    
    if len(ma50.dropna()) > 0 and len(ma200.dropna()) > 0:
        ma50_val = float(ma50.dropna().iloc[-1])
    ...
    if len(bb_lower.dropna()) > 0 and len(bb_upper.dropna()) > 0:
...
def classify_signal(df, direction, score, btc_change_24h=None):
    ...
    try:
        ...
        # Simple RSI calculation for classification
        rsi_series = []
    ...
    for t in trades:
        ...
        try:
            ...
            if direction == "LONG":
                ...
                if price <= t["sl"]:
                    t["status"]   = "LOSS"
                    t["pnl_pct"]  = round((t["sl"] - entry) / entry * 100 * LEVERAGE, 2)
                    t["pnl_usdt"] = round(ACCOUNT_SIZE * RISK_PCT / 100 * t["pnl_pct"] / 100, 2)
                    closed_now.append(t)
                elif price >= t["tp2"]:
                    t["status"]   = "WIN"
                    t["pnl_pct"]  = round((t["tp2"] - entry) / entry * 100 * LEVERAGE, 2)
                    t["pnl_usdt"] = round(ACCOUNT_SIZE * RISK_PCT / 100 * t["pnl_pct"] / 100, 2)
                    closed_now.append(t)
                else:
                    t["pnl_pct"]  = round(pnl_pct, 2)
                ...
                if price >= t["sl"]:
                    t["status"]   = "LOSS"
                    t["pnl_pct"]  = round((entry - t["sl"]) / entry * 100 * LEVERAGE, 2)
                    t["pnl_usdt"] = round(ACCOUNT_SIZE * RISK_PCT / 100 * t["pnl_pct"] / 100, 2)
                    closed_now.append(t)
                elif price <= t["tp2"]:
                    t["status"]   = "WIN"
                    t["pnl_pct"]  = round((entry - t["tp2"]) / entry * 100 * LEVERAGE, 2)
                    t["pnl_usdt"] = round(ACCOUNT_SIZE * RISK_PCT / 100 * t["pnl_pct"] / 100, 2)
                    closed_now.append(t)
                else:
                    t["pnl_pct"]  = round(pnl_pct, 2)
...
async def cmd_scalp(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        scalp_results = await asyncio.get_event_loop().run_in_executor(None, lambda: asyncio.run(scalp_scan_coins()))
                    ...
                    # Trailing stop management after TP1 hit
                        ...
                        # Auto TP2 adjustment — FIX: only move TP2 in favorable direction
                            ...
                            try:
                                ...
                                tr = pd.concat([high_low, high_close, low_close], axis=1).max(axis=1)
                                atr_fresh = tr.rolling(14).mean()
                                new_atr   = float(atr_fresh.iloc[-1]) if len(atr_fresh.dropna()) > 0 else atr
