...
GEMINI_MAX_PER_CYCLE   = 3
GEMINI_MAX_PER_DAY     = 1400
_gemini_cycle_count:   int   = 0
_gemini_cycle_reset:   float = 0.0
_GEMINI_STATE_FILE     = "/tmp/gemini_state.json"  # persists across restarts

def _gemini_load_state():
    """Load Gemini quota state from disk on startup — prevents reset burning daily quota."""
    global _gemini_blocked_until, _gemini_daily_count, _gemini_daily_reset
    try:
        import json as _json
        with open(_GEMINI_STATE_FILE, "r") as _f:
            _s = _json.load(_f)
        # Only restore if the daily window hasn't expired
        if time.time() - _s.get("daily_reset", 0) < 86400:
            _gemini_daily_count = _s.get("daily_count", 0)
            _gemini_daily_reset = _s.get("daily_reset", 0)
        # Restore backoff only if still active
        _bu = _s.get("blocked_until", 0)
        if _bu > time.time():
            _gemini_blocked_until = _bu
            log.info(f"Gemini backoff restored — blocked for {_bu - time.time():.0f}s more")
    except Exception:
        pass  # no state file yet — first run

def _gemini_save_state():
    """Save Gemini quota state to disk."""
    try:
        import json as _json
        with open(_GEMINI_STATE_FILE, "w") as _f:
            _json.dump({
                "daily_count":   _gemini_daily_count,
                "daily_reset":   _gemini_daily_reset,
...
def _gemini_set_backoff(retry_after_seconds: float = 30.0):
    ...
    _gemini_blocked_until = time.time() + retry_after_seconds + 2.0
