...
def check_15m_entry_quality(df_15m: pd.DataFrame, direction: str) -> dict:
    ...
    try:
        ...
        if direction == "LONG":
            ...
            # ENTER signals — widened from RSI < 45 to < 58 to cover normal entry conditions
            if rsi_v < 58:
            ...
            if -2.0 < dist_ema9_pct < 1.5:
                enter_reasons.append(f"Price near 15m EMA9 ({dist_ema9_pct:+.1f}%) — clean entry zone")
        else:
            if rsi_v < 32:
            ...
            # ENTER signals — widened from RSI > 55 to > 42
            if rsi_v > 42:
