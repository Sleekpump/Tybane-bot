...
def score_structure_group(df: pd.DataFrame, regime: dict) -> tuple[float, list[str]]:
    ...
    smc_pts = int(smc_s * 3)   # map -1..+1 → -3..+3 to match old candle weighting
    points += smc_pts
    total  += 3
...
def compute_signal_quality(
    ...
    # --- GROUP B: MOMENTUM ---
    # Removed: score_oscillator_group (StochRSI, CCI, Williams %R).
    # These are all RSI derivatives — redundant with RSI + MBI already in
    # score_momentum_group, and they lag. Momentum vote is now 100% score_momentum_group.
    mom_s,  mom_sigs = score_momentum_group(df_ltf, regime)
    osc_s = 0.0      # kept for group_scores key compatibility — no longer computed
...
def score_timeframe_v2(df: pd.DataFrame, regime: dict = None) -> tuple:
    ...
    # score_oscillator_group removed — StochRSI/CCI/Williams%R are lagging RSI
    # derivatives already covered by score_momentum_group. Its 10pt weight
    # has been redistributed to structure (now 30) which covers SMC signals.
    osc_s = 0.0
...
def check_15m_entry_quality(df_15m: pd.DataFrame, direction: str) -> dict:
    """
    ...
    try:
        ...
        if macd_df is not None and not macd_df.empty and len(macd_df) >= 3:
            ml_now  = float(macd_df.iloc[-1, 0])
            sl_now  = float(macd_df.iloc[-1, 2])
            ml_prev = float(macd_df.iloc[-2, 0])
            sl_prev = float(macd_df.iloc[-2, 2])
        ...
        if direction == "LONG":
            ...
            if dist_ema9_pct > 3.0:
                ...
                pullback_target = round(ema9_v * 1.005, 6)
            ...
            if dist_ema9_pct < -3.0:
                ...
                pullback_target = round(ema9_v * 0.995, 6)
