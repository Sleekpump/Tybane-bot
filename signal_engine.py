...
def score_timeframe_v2(df: pd.DataFrame, regime: dict = None) -> tuple:
    ...
    if regime is None:
        # Fallback — logs a warning so any future caller without regime is visible.
        # In production this should never fire since analyze_v2 always passes regime.
        log.warning("score_timeframe_v2 called without regime — detecting from df directly. "
                    "Pass 4H-based regime explicitly to avoid incorrect context.")
