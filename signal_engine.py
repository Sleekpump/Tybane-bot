...
def _empty_zlema(price: float) -> dict:
    return {"zlema": price, "trend": 1, "bullish": True, "bearish": False,
...
def _empty_vidya(price: float) -> dict:
    return {"vidya": price, "trend": 1, "bullish": True, "bearish": False,
            "just_flipped": False, "volume_delta": 0.0, "delta_bullish": True, "k_value": 0.0}
...
def compute_mbi(df: pd.DataFrame, length: int = 14, signal_length: int = 9) -> dict:
    ...
    try:
        ...
        atr_clean = atr_v.bfill().replace(0, np.nan).fillna(method=None).bfill()
...
# ─── ZERO LAG EMA — confirmed AlgoAlpha formula ──────────────────────────────
def _empty_zlema(price: float) -> dict:
    return {"zlema": price, "trend": 1, "bullish": True, "bearish": False,
            "just_flipped": False, "entry_long": False, "entry_short": False,
            "upper_band": price, "lower_band": price, "volatility": 0.0}
...
# ─── VOLUMATIC VIDYA — Tushar Chande / BigBeluga formula ─────────────────────
def _empty_vidya(price: float) -> dict:
    return {"vidya": price, "trend": 1, "bullish": True, "bearish": False,
            "just_flipped": False, "volume_delta": 0.0,
            "delta_bullish": True, "k_value": 0.0}
...
def _empty_mbi() -> dict:
    return {"histogram": 0.0, "signal": 0.0, "momentum": 0.0,
            ...
            "bright_green": False, "bright_red": False}
