...
def volatility_check(df: pd.DataFrame) -> dict:
    """
    Returns {"pass": bool, "atr_pct": float, "atr_percentile": float, "reason": str}

    Adaptive ATR threshold — uses percentile of recent ATR distribution
    rather than a fixed 8% ceiling. Fixes the BTC-vs-meme-coin problem:
    a 4% ATR is normal for DOGE but extreme for BTC.

    Logic:
      - Compute ATR% for last 100 candles
      - If current ATR% is in the top 85th percentile of its own history → too erratic
      - If current ATR% is in the bottom 10th percentile → too dead
      - Hard floor: still reject if ATR% < VOLATILITY_MIN_ATR_PCT (absolute dead market)
    """
    close = df["close"]
    high  = df["high"]
    low   = df["low"]
    ...
    if atr_series is not None and not atr_series.empty:
        _raw = float(atr_series.iloc[-1])
        atr_v = 0.0 if np.isnan(_raw) else _raw

    # Hard-fail if ATR is 0 or unavailable
    if atr_v <= 0 or price <= 0:
        return {"pass": False, "atr_pct": 0.0, "atr_percentile": 0.0,
                "reason": "ATR unavailable — insufficient data"}

    atr_pct = atr_v / price * 100

    # ── Absolute dead-market floor (still needed — percentile can't catch this) ─
    if atr_pct < VOLATILITY_MIN_ATR_PCT:
        return {"pass": False, "atr_pct": round(atr_pct, 2), "atr_percentile": 0.0,
                "reason": f"ATR too low ({atr_pct:.2f}%) — no movement"}

    # ── Compute adaptive percentile from recent ATR history ──────────────────
    # Use last 100 bars of ATR% to build the distribution for this specific asset
    atr_percentile = 50.0  # fallback if not enough history
    try:
        if len(atr_series.dropna()) >= 30:
            # ATR% series for the full available window
            recent_atrs   = atr_series.dropna().tail(100)
            recent_prices = close.loc[recent_atrs.index]
            atr_pct_hist  = (recent_atrs / recent_prices * 100).dropna().values
            if len(atr_pct_hist) >= 10:
                atr_percentile = float(np.sum(atr_pct_hist <= atr_pct) / len(atr_pct_hist) * 100)
    except Exception:
        pass  # fall through to fixed-threshold fallback

    # ── Adaptive decision ──────────────────────────────────────────────────────
    # 85th percentile = top 15% of this asset's own volatility history → too erratic
    # 10th percentile = bottom 10% → sleeping, skip
    # Fallback to fixed ceiling only when history is insufficient
    if atr_percentile >= 85.0:
        return {"pass": False, "atr_pct": round(atr_pct, 2), "atr_percentile": round(atr_percentile, 1),
                "reason": f"ATR in {atr_percentile:.0f}th pct of own history ({atr_pct:.1f}%) — abnormally erratic"}
    if atr_percentile <= 10.0:
        return {"pass": False, "atr_pct": round(atr_pct, 2), "atr_percentile": round(atr_percentile, 1),
                "reason": f"ATR in {atr_percentile:.0f}th pct of own history ({atr_pct:.2f}%) — abnormally quiet"}
    # Fixed ceiling as hard backstop for assets with very short history
    if atr_pct > VOLATILITY_MAX_ATR_PCT:
        return {"pass": False, "atr_pct": round(atr_pct, 2), "atr_percentile": round(atr_percentile, 1),
                "reason": f"ATR too high ({atr_pct:.1f}%) — absolute ceiling exceeded"}

    return {"pass": True, "atr_pct": round(atr_pct, 2), "atr_percentile": round(atr_percentile, 1),
            "reason": f"OK ({atr_percentile:.0f}th pct)"}
...
def score_timeframe_v2(df: pd.DataFrame, regime: dict = None) -> tuple:
    ...
    IMPORTANT: always pass regime explicitly from a 4H dataframe.
    The fallback detect_regime(df) will compute regime from whatever TF
    df is — if df is 1H, regime will be wrong (too noisy). The fallback
    exists only for legacy/standalone callers; all internal callers
    (analyze_v2, compute_signal_quality) pass regime explicitly.
    """
    close  = df["close"]
    high   = df["high"]
    low    = df["low"]
    ...
    if regime is None:
        # Fallback — should never fire in production since analyze_v2 always
        # passes regime explicitly from 4H data. If it does fire, log clearly.
        # We cannot raise here (would break legacy callers) but the warning
        # makes the problem immediately visible in Railway logs.
        log.warning(
            "score_timeframe_v2 called without regime — falling back to df-based detection. "
            "This produces incorrect regime context if df is 1H not 4H. "
            "Fix: always pass regime=detect_regime(df_4h) at the call site."
        )
        regime = detect_regime(df)
    # Guard: ensure regime is a valid dict with required keys
    if not isinstance(regime, dict) or "regime" not in regime:
        log.warning("score_timeframe_v2: invalid regime dict — using RANGING default")
        regime = {"regime": "RANGING", "adx": 0, "trend": "NEUTRAL"}
...
def analyze_v2(symbol: str, fetch_ohlcv_fn, coin_labels: dict,
               funding_rate: float = 0.0) -> dict:
    """
    Full Phase 1 analysis — called by ai_validator.run_full_pipeline().

    funding_rate: pass live funding rate so check_exhaustion sees real data.
    Previously hardcoded to 0.0 — funding-based exhaustion signals never fired.
...
def detect_liquidity_pools(
    ...
    try:
        ...
        # ── Sweep detection — tightened to 2 bars ────────────────────────────
        # The actual entry signal is: wick pierced below SSL, price CLOSED back
        # above within 2 candles. 4 bars was too loose — that's 4H of potential
        # noise on 1H data. The X marks on SMC charts happen within 1-2 candles.
        sweep_bullish  = False
        sweep_bearish  = False
        sweep_bars_ago = None   # how many bars since the sweep (for recency context)

        if ssl_level > 0 and n >= 3:
            for _i in range(1, 3):   # check last 2 bars only
                if low[-_i] < ssl_level * (1 - tol) and price > ssl_level:
                    sweep_bullish  = True
                    sweep_bars_ago = _i
                    break

        if bsl_level > 0 and n >= 3:
            for _i in range(1, 3):
                if high[-_i] > bsl_level * (1 + tol) and price < bsl_level:
                    sweep_bearish  = True
                    sweep_bars_ago = _i
                    break
...
def detect_poi_density(
    ...
    """
    POI (Point of Interest) Density Scoring.

    A POI is not a single level — it's a zone where multiple structure
    elements overlap at the same price. The more layers that stack within
    tolerance_pct of each other, the stronger the zone.

    Checks 4 layers:
      1. Order Block (demand OB for LONG, supply OB for SHORT)
      2. FVG (Fair Value Gap) mid within tolerance of target
      3. Previous swing high/low that flipped to support/resistance
      4. Liquidity pool (equal lows for LONG, equal highs for SHORT)

    Returns:
    {
        "density":        int,    0-4 layers overlapping at this price
        "label":          str,    "Strong POI" / "Moderate POI" / "Weak zone" / "No confluence"
        "layers":         list,   which layers are present
        "ob_at_level":    bool,
        "fvg_at_level":   bool,
        "struct_at_level":bool,
        "liq_at_level":   bool,
    }
    """
    EMPTY = {"density": 0, "label": "No confluence", "layers": [],
             "ob_at_level": False, "fvg_at_level": False,
             "struct_at_level": False, "liq_at_level": False}
    try:
        if df is None or len(df) < 20 or target_price <= 0:
            return EMPTY
        ...
        def _near(level: float) -> bool:
            return level > 0 and abs(level - target_price) / target_price <= tol

        # ── Layer 1: Order Block ──────────────────────────────────────────────
        ob_data     = detect_order_blocks(df)
        ob_at_level = False
        if direction == "LONG":
            dob = ob_data.get("demand_ob")
            if dob and dob.get("active"):
                if _near(dob["high"]) or _near(dob["low"]):
                    ob_at_level = True
                    layers.append("OB")
        else:
            sob = ob_data.get("supply_ob")
            if sob and sob.get("active"):
                if _near(sob["high"]) or _near(sob["low"]):
                    ob_at_level = True
                    layers.append("OB")

        # ── Layer 2: FVG (Fair Value Gap) ────────────────────────────────────
        fvg_at_level = False
        try:
            for i in range(1, len(high) - 1):
                if direction == "LONG" and high[i-1] < low[i+1]:
                    mid = (high[i-1] + low[i+1]) / 2
                    if _near(mid):
                        fvg_at_level = True
                        layers.append("FVG")
                        break
                elif direction == "SHORT" and low[i-1] > high[i+1]:
        ...
        # ── Layer 3: Previous structure high/low flipped S/R ─────────────────
        struct_at_level = False
        sh, sl = _find_swing_points(df, left=2, right=2, lookback=60)
        if direction == "LONG":
            for _, p in sl:
                if _near(p):
                    struct_at_level = True
                    layers.append("Prev swing low")
                    break
            if not struct_at_level:
                for _, p in sh:
                    if _near(p):
                        struct_at_level = True
                        layers.append("Prev swing high (flipped)")
                        break
        else:
            for _, p in sh:
                if _near(p):
                    struct_at_level = True
                    layers.append("Prev swing high")
                    break
            if not struct_at_level:
                for _, p in sl:
                    if _near(p):
                        struct_at_level = True
                        layers.append("Prev swing low (flipped)")
                        break

        # ── Layer 4: Liquidity pool ───────────────────────────────────────────
        liq_data    = detect_liquidity_pools(df)
        liq_at_level = False
        if direction == "LONG":
            ssl = liq_data.get("equal_lows_level", 0)
            if _near(ssl):
                liq_at_level = True
                layers.append("Liquidity pool (SSL)")
        else:
            bsl = liq_data.get("equal_highs_level", 0)
            if _near(bsl):
                liq_at_level = True
                layers.append("Liquidity pool (BSL)")
        ...
        return {
            ...
            "struct_at_level": struct_at_level,
            "liq_at_level":    liq_at_level,
        }

    except Exception as e:
        log.warning(f"detect_poi_density error: {e}")
        return EMPTY


def smc_structure_score(df: pd.DataFrame, regime: dict) -> tuple:
    """
...
def grade_signal(
    ...
    try:
        ...
        if bb is not None and not bb.empty and len(bb) >= 10:
            # Use named BBB column — positional iloc[N, 3] breaks if pandas_ta
            # changes column order (same vulnerability fixed in ADX/MACD)
            _bbb_col = [c for c in bb.columns if c.startswith("BBB_")]
            if _bbb_col:
                bw_now  = float(bb[_bbb_col[0]].iloc[-1])
                bw_prev = float(bb[_bbb_col[0]].iloc[-6])
    ...
    try:
        ...
        if bb is not None and not bb.empty and len(bb) >= 10:
            try:
                ...
                if _bbb_col:
                    bw_now  = float(bb[_bbb_col[0]].iloc[-1])
                    bw_prev = float(bb[_bbb_col[0]].iloc[-10])
