...
def analyze_v2(symbol: str, fetch_ohlcv_fn, coin_labels: dict) -> dict:
    ...
    # funding_rate populated by run_full_pipeline after this returns
    # Pass 0.0 here — regime gate SHORT filter uses it but caller overwrites r["funding"] anyway
    # Real impact: SHORT signals in TRENDING_DOWN need funding_rate > 0
    # This is acceptable — the AI validator sees the real funding rate via the prompt
...
# ─── BLOW-OFF TOP / EXHAUSTION FILTER ────────────────────────────────────────
def check_exhaustion(
    ...
    try:
        ...
        if blowoff_signals >= 3:
            block_long = True
            reasons.append(f"→ Blow-off top detected ({blowoff_signals}/4 signals)")

        # ── SHORT opportunity — after extreme pump ────────────────────────────
        short_signals = 0
        if change_24h >= 25 and vol_collapsing:
            short_signals += 2
            reasons.append(f"Pump {change_24h:.1f}% + volume collapsing — distribution")
        if funding_rate >= 0.05:
            short_signals += 2
            reasons.append(f"Funding {funding_rate*100:.3f}% — crowded longs, squeeze risk")
        if rsi_val >= 78 and ext_pct >= 12:
            short_signals += 2
            reasons.append(f"RSI {rsi_val:.0f} + {ext_pct:.1f}% above EMA50 — extreme overextension")
        if change_24h >= 30:
            short_signals += 1
            reasons.append(f"30%+ single-day pump — blow-off territory")

        if short_signals >= 2:
            short_opp  = True
            block_long = True  # always block LONG if SHORT opportunity exists
