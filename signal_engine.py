...
def compute_supertrend(
    ...
    try:
        ...
        if atr_s is None or len(atr_s.dropna()) < 5:
            return _empty_supertrend(float(close[-1]))
        atr_v  = atr_s.fillna(method="bfill").values
...
def grade_signal(
    ...
    # ── Supertrend filter — hard block + grade bonus ─────────────────────────
    ...
    try:
        ...
        # Supertrend scoring — graduated, not binary
        if direction == "LONG":
            if st_res["bullish"] and st_htf["bullish"]:
                grade_score += 2
                reasons.append(f"Supertrend bullish both TFs (price {st_res['dist_pct']:+.1f}%)")
                if st_res["just_flipped"]:
                    grade_score += 1
                    reasons.append("Supertrend just flipped bullish — fresh signal")
            elif st_res["bullish"]:
                grade_score += 1
                reasons.append(f"Supertrend bullish LTF (price {st_res['dist_pct']:+.1f}%)")
            elif st_res["bearish"] and st_htf["bearish"]:
                grade_score -= 2
                warnings.append("Supertrend bearish both TFs — LONG opposed")
            else:
                grade_score -= 1
                warnings.append("Supertrend bearish LTF — caution on LONG")
        else:  # SHORT
            if st_res["bearish"] and st_htf["bearish"]:
                grade_score += 2
                reasons.append(f"Supertrend bearish both TFs (price {st_res['dist_pct']:+.1f}%)")
                if st_res["just_flipped"]:
                    grade_score += 1
                    reasons.append("Supertrend just flipped bearish — fresh signal")
            elif st_res["bearish"]:
                grade_score += 1
                reasons.append(f"Supertrend bearish LTF (price {st_res['dist_pct']:+.1f}%)")
            elif st_res["bullish"] and st_htf["bullish"]:
                grade_score -= 2
                warnings.append("Supertrend bullish both TFs — SHORT opposed")
            else:
                grade_score -= 1
                warnings.append("Supertrend bullish LTF — caution on SHORT")
    except Exception as e:
        log.warning("Supertrend grade integration error: " + str(e))
