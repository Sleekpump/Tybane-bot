...
def grade_signal(
    ...
    try:
        ...
        # Hard blocks — signal opposes both LTF and HTF Supertrend
        if direction == "LONG" and st_res["bearish"] and st_htf["bearish"]:
            warnings.append("Supertrend BEARISH on both timeframes — LONG opposed")
            grade_score -= 3
        elif direction == "SHORT" and st_res["bullish"] and st_htf["bullish"]:
            warnings.append("Supertrend BULLISH on both timeframes — SHORT opposed")
            grade_score -= 3
        # Grade bonus — signal aligns with Supertrend direction
        elif direction == "LONG" and st_res["bullish"]:
            reasons.append(f"Supertrend bullish (price {st_res['dist_pct']:+.1f}% above line)")
            grade_score += 2
            if st_res["just_flipped"]:
                grade_score += 1
                reasons.append("Supertrend just flipped bullish — fresh signal")
        elif direction == "SHORT" and st_res["bearish"]:
            reasons.append(f"Supertrend bearish (price {st_res['dist_pct']:+.1f}% below line)")
            grade_score += 2
            if st_res["just_flipped"]:
                grade_score += 1
