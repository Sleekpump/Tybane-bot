    ...
    for symbol, label in list(COIN_LABELS.items()):
        ...
        try:
            ...
            if change_4h >= 5.0 or change_1h >= 4.0:
                ...
                _cs_fund_score = (15 if _cs_funding_pct > 0.05       # longs paying → SHORT aligned
                                  else 5 if abs(_cs_funding_pct) <= 0.05   # neutral
                                  else 0)                              # shorts paying → SHORT misaligned
                _cs_conviction = min(100, round(_cs_rr_score + _cs_den_score + _cs_vol_score + _cs_fund_score))
            ...
            # ── Check for SHORT→LONG counter (coin dumped, bounce back to OB) ─
                ...
                _cs_den_score  = {3: 30, 2: 15}.get(min(_cs_density, 3), 0)
                _cs_vol_score  = max(0, round(20 - (_vol_ratio - 0.5) * 13.3))
                _cs_fund_score = (15 if _cs_funding_pct < -0.05      # shorts paying → LONG aligned
                                  else 5 if abs(_cs_funding_pct) <= 0.05   # neutral
                                  else 0)                              # longs paying → LONG misaligned
                _cs_decouple   = 10 if _is_decoupled else 0
                _cs_conviction = min(100, round(_cs_rr_score + _cs_den_score + _cs_vol_score + _cs_fund_score + _cs_decouple))
