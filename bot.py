...
async def cmd_watch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Alert threshold: quality ≥ " + str(quality_min) + " or regime flip.\n\n"
...
async def auto_watch(app):
    ...
    Swing: quality check on 4H+1D — reversal must show on 4H candle (quality >= 65)
    Scalp: quality check on 1H+4H — current behaviour (quality >= 55)
    Regime flip always triggers on 4H regardless of trade type.
    """
    ...
    while True:
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                # Use per-coin threshold (swing=65, scalp=55)
                ...
                if not regime_flip and not reversal_detected:
                    continue
                ...
                if regime_flip:
                    trigger_str = "Regime changed to `" + regime_name + "` (ADX: " + str(round(adx_val, 1)) + ")"
                else:
                    confidence = quality_result.get("confidence", "MEDIUM")
                    trigger_str = "Reversal quality: `" + str(round(quality_score, 1)) + "/100` | " + confidence + " | " + trade_type_label + " (" + tf_label + ")"
