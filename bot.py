...
async def cmd_watch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    watched_trades[symbol] = {
        ...
        "trail_step_pct":     5.0,    # alert every 5% move in our favour
    }
    ...
    trail_note = "\n\U0001f4cd Trailing SL: alerts every 5% move in your favour." if entry_price else ""
    await update.message.reply_text(
        "\U0001f440 *Now watching " + coin + "*\n\n"
        "Direction: *" + direction + "* | Type: *" + trade_type.upper() + "*" + entry_str + "\n"
        + conf_notes.get(confidence, "") + trail_note + "\n\n"
    ...
    while True:
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                # ── Trailing SL check — runs every cycle regardless of trade type ──
                entry      = w.get("entry")
                trail_peak = w.get("trail_peak")
                trail_sl   = w.get("trail_sl")
                step_pct   = w.get("trail_step_pct", 5.0)
                trail_cd   = 1800  # 30 min between trail SL alerts

                if entry and entry > 0:
                    try:
                        _df_trail = fetch_ohlcv(symbol, "1h", 5)
                        ...
                        # How far have we moved in our favour from entry?
                        if direction == "LONG":
                            gain_pct = (new_peak - entry) / entry * 100
                        else:
                            gain_pct = (entry - new_peak) / entry * 100

                        # Suggest SL at 50% of the gain — locks in half the profit
                        # e.g. entry 1.00, peak 1.10 (+10%) → SL at 1.05 (+5%)
                        if gain_pct >= step_pct:
                            lock_pct = gain_pct * 0.5  # lock in 50% of the move
                            if direction == "LONG":
                                new_sl = round(entry * (1 + lock_pct / 100), 4)
                            else:
                                new_sl = round(entry * (1 - lock_pct / 100), 4)

                            # Only alert if SL improved significantly (0.5%+ better than last)
                            sl_improved = (
                                trail_sl is None or
                                (direction == "LONG"  and new_sl > trail_sl * 1.005) or
                                (direction == "SHORT" and new_sl < trail_sl * 0.995)
                            )
                            trail_cd_ok = now - w.get("trail_last_alert", 0) >= trail_cd

                            if sl_improved and trail_cd_ok:
                                watched_trades[symbol]["trail_sl"]         = new_sl
                                watched_trades[symbol]["trail_last_alert"] = now

                                pnl_now = (cur_price - entry) / entry * 100 * LEVERAGE if direction == "LONG"                                           else (entry - cur_price) / entry * 100 * LEVERAGE

                                msg  = "\U0001f4cd *Trail SL Update: " + label + "*\n\n"
                                msg += ("\U0001f7e2" if direction == "LONG" else "\U0001f534")
                                msg += " *" + direction + "* from `$" + "{:.4f}".format(entry) + "`\n"
                                msg += "Current: `$" + "{:.4f}".format(cur_price) + "` | Peak: `$" + "{:.4f}".format(new_peak) + "`\n"
                                msg += "PnL: *" + "{:+.1f}".format(pnl_now) + "%* at " + str(LEVERAGE) + "x\n\n"
                                msg += "\U0001f6e1 *Move your SL to: `$" + "{:.4f}".format(new_sl) + "`*\n"
                                msg += "_Locks in " + "{:.1f}".format(lock_pct) + "% of your " + "{:.1f}".format(gain_pct) + "% gain_\n"
                                msg += "_(" + "{:.1f}".format(gain_pct * 0.5 * LEVERAGE) + "% at " + str(LEVERAGE) + "x protected)_"
                                try:
                                    await send_msg(app, msg)
                                except Exception:
                                    await app.bot.send_message(chat_id=TELEGRAM_CHAT, text=msg, parse_mode=None)
                                log.info("Trail SL alert: " + label + " SL→$" + str(new_sl) + " | gain=" + str(round(gain_pct,1)) + "%")
                    except Exception as e:
                        log.warning("Trail SL check error " + label + ": " + str(e))
