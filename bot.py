...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # User can pass 5th arg: /refine BTC LONG 95000 10 scalp
    # Otherwise inferred from leverage: >=10x = scalp, <10x = swing
    ...
    if _type_arg in ("scalp", "s"):
        ...
        trade_type = "scalp" if leverage >= 10 else "swing"
    ...
    try:
        ...
        # ── SL — timeframe aware: scalp uses 1H, swing uses 4H ──────────
        sl_price = None
        try:
        ...
        try:
            ...
            if direction == "LONG":
                tp1_price = round(entry_price + _atr_tp * _m1, 6)
                tp2_price = round(entry_price + _atr_tp * _m2, 6)
            else:
                tp1_price = round(entry_price - _atr_tp * _m1, 6)
                tp2_price = round(entry_price - _atr_tp * _m2, 6)
        ...
        if strong_trend_override and action in ("WAIT", "CONDITIONAL", "NEUTRAL"):
            entry_check["action"] = "ENTER"
            action      = "ENTER"
            action_icon = "✅"
            entry_check["reason"] = trend_strength_note
        ...
        if entry_check.get("pullback_target") and action in ("WAIT", "CONDITIONAL"):
            msg += f"🎯 Entry zone: ${entry_check['pullback_target']:.4f}\n"
        else:
            msg += f"🎯 Entry: ${entry_price} (now)\n"

        # OB reference — always show if active, regardless of verdict
        if _ob_ref:
            msg += (f"📍 {_ob_label}: ${_ob_ref['low']:.4f} – ${_ob_ref['high']:.4f}"
                    f" ({'at zone' if _ob_r.get('price_at_demand_ob') or _ob_r.get('price_at_supply_ob') else 'nearby'})\n")

        if sl_price:
            sl_pct = abs(sl_price - entry_price) / entry_price * 100
            _sl_tf = "1H" if trade_type == "scalp" else "4H"
            msg += f"🛑 SL ({_sl_tf} structure): ${sl_price} ({sl_pct:.1f}% away)\n"

        if tp1_price and tp2_price:
            msg += f"✅ TP1: ${tp1_price:.4f} (+{tp1_pct_r}%)\n"
            msg += f"✅ TP2: ${tp2_price:.4f} (+{tp2_pct_r}%)\n"

        # Watch command — trade_type aware
        _watch_tp   = f" {tp1_pct_r} {tp2_pct_r}" if (tp1_pct_r and tp2_pct_r) else ""
        _watch_type = trade_type   # "scalp" or "swing"
        msg += f"\n`/watch {raw_symbol} {direction} {entry_price} {_watch_type} {leverage}{_watch_tp}`"
