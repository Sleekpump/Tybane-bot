        ...
        for symbol, w in list(rescan_watches.items()):
            try:
                ...
                if cd_left > 0:
                    ...
                    # Don't update _last_checked — cooldown means we didn't attempt
                    continue
                ...
                if watch_price and watch_price > 0:
                    try:
                        ...
                        pass  # price fetch failed — proceed with pipeline anyway

                # ── Gate 3: full pipeline ─────────────────────────────────────
                log.info(f"Rescan: running pipeline for {label} {direction}...")
                ...
                # Rate-limit between pipeline calls — each analyze_async fetches
                # 3+ OHLCV timeframes from Bitget. Pause after every call so a
                # multi-coin rescan cycle never fires them back-to-back.
                await asyncio.sleep(REQUEST_DELAY * 5)

                # ── Gate 4: direction match ───────────────────────────────────
                if _rsc_det_dir != direction:
                    log.info(
                        f"Rescan {label}: pipeline→{_rsc_det_dir} | "
                        f"watching→{direction} — mismatch, skipping"
                    )
                    rescan_watches[symbol]["_blocked_at"] = (
                        f"Gate 4 — Direction mismatch: pipeline says {_rsc_det_dir}, "
                        f"watching {direction}"
                    )
                    continue

                # ── Gate 5: quality threshold ─────────────────────────────────
                _rsc_q_min = w.get("quality_min", RESCAN_QUALITY_MIN)
