...
WATCH_COOLDOWN      = 1800 # 30 min cooldown between alerts per coin
...
async def analyze_async(symbol):
    try:
        ...
        log.info(result["label"] + ": " + result["direction"] + " | Q:" + str(result.get("abs_score", 0)) + " | " + result.get("signal_type", "?"))
        return result
    except Exception as e:
...
def get_actionable(results):
    # Only MOMENTUM signals fire — REVERSAL disabled (0-20% win rate observed)
    # WEAK signals always excluded
...
async def cmd_coin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Use calc_levels_v2 for consistent ATR-adjusted levels
        from risk_manager import calc_levels_v2
        atr_pct = (r["atr"] / r["price"] * 100) if r["price"] > 0 else 1.0
        sl, tp1, tp2, _ = calc_levels_v2(r["direction"], r["price"], r["atr"], atr_pct)
...
async def cmd_swing(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if actionable:
            ...
            sl, tp1, tp2, _ = calc_levels_v2(best["direction"], best["price"], best["atr"], _atr_pct)
            ob_bias, ob_ratio = get_order_book_bias(best["symbol"])
                    ...
                    # Trailing stop management after TP1 hit
                        ...
                        # Auto TP2 adjustment — FIX: only move TP2 in favorable direction
                            ...
                            try:
                                ...
                                _, _, new_tp2, _ = calc_levels_v2(direction, price, new_atr, atr_pct)
                                old_tp2   = sig["tp2"]
...
async def cmd_watch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Alert: regime flip AND quality ≥ " + str(quality_min) + " must both agree.\n\n"
...
async def cmd_watching(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol, w in watched_trades.items():
        ...
        q_min      = w.get("quality_min", WATCH_QUALITY_MIN_SWING)
        msg += "• *" + w["label"] + "* " + direction + " " + type_label + " | Entry: " + entry_str + " | Q≥" + str(q_min)
        if price:
...
async def auto_watch(app):
    ...
    Fires ONLY when BOTH conditions agree simultaneously (Option C):
      1. Regime has flipped against your trade direction (4H ADX-based)
      2. Quality score >= threshold in reversal direction (swing=65, scalp=55)
    Swing quality: 4H+1D timeframes. Scalp quality: 1H+4H timeframes.
    Single trigger alone = no alert. Both required = high confidence reversal.
    """
    ...
    while True:
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                # Run Phase 1 on the coin
                from signal_engine import detect_regime, compute_signal_quality
                import pandas_ta as _ta

                is_swing = w.get("trade_type", "swing") == "swing"

                # Regime detection — always on 4H regardless of trade type
                df_4h_regime = fetch_ohlcv(symbol, "4h", 100)
                regime       = detect_regime(df_4h_regime)
                regime_name  = regime.get("regime", "RANGING")
                adx_val      = regime.get("adx", 0)

                regime_flip = (
                    (direction == "LONG"  and regime_name == "TRENDING_DOWN") or
                    (direction == "SHORT" and regime_name == "TRENDING_UP")
                )

                # Quality check timeframes:
                #   Swing: 4H as LTF + 1D as HTF — reversal must show on 4H candle
                #   Scalp: 1H as LTF + 4H as HTF — current behaviour
                if is_swing:
                    df_ltf = fetch_ohlcv(symbol, "4h", 200)
                    df_htf = fetch_ohlcv(symbol, "1d", 100)
                else:
                    df_ltf = fetch_ohlcv(symbol, "1h", 200)
                    df_htf = df_4h_regime  # reuse already-fetched 4H

                price       = float(df_ltf["close"].iloc[-1])
                rsi_s       = _ta.rsi(df_ltf["close"], length=14)
                rsi_current = float(rsi_s.dropna().iloc[-1]) if rsi_s is not None and len(rsi_s.dropna()) > 0 else 50.0
                quality_result = compute_signal_quality(df_ltf, df_htf, symbol, funding_rate=0.0, rsi_val=rsi_current)

                reversal_direction = "LONG" if direction == "SHORT" else "SHORT"
                signal_direction   = quality_result.get("direction", "NEUTRAL")
                quality_score      = quality_result.get("quality_score", 0)

                # Option C — BOTH triggers must agree to fire
                # Regime alone is too noisy. Quality alone misses structure.
                # Together they confirm a real reversal.
                coin_quality_min = w.get("quality_min", WATCH_QUALITY_MIN_SWING)

                quality_against_trade = (
                    signal_direction == reversal_direction and
                    quality_score >= coin_quality_min
                )

                # Alert only when BOTH agree simultaneously
                alert_should_fire = regime_flip and quality_against_trade

                if not alert_should_fire:
                    continue
                ...
                if entry and price:
                    ...
                    pnl_str = "PnL: " + "{:+.2f}".format(pnl_raw) + "% (" + "{:+.1f}".format(pnl_lev) + "% at " + str(LEVERAGE) + "x)\n"

                trade_type_label = w.get("trade_type", "swing").upper()
                tf_label         = "4H+1D" if is_swing else "1H+4H"
                confidence       = quality_result.get("confidence", "MEDIUM")
                # Both triggers confirmed — show both in the alert
                trigger_str = (
                    "Regime: `" + regime_name + "` (ADX: " + str(round(adx_val, 1)) + ")"
                    + " + Reversal quality: `" + str(round(quality_score, 1)) + "/100`"
                    + " | " + confidence + " | " + trade_type_label + " (" + tf_label + ")"
                )

                entry_str = "$" + "{:.4f}".format(entry) if entry else "not set"

                msg  = "⚠ *Reversal Warning: " + label + "*\n\n"
                msg += "Your position: *" + direction + "* from " + entry_str + "\n"
                msg += "Current price: `$" + "{:.4f}".format(price) + "`\n"
                msg += pnl_str
                msg += "\nTrigger: " + trigger_str + "\n"
                msg += "\n💡 Consider closing your *" + direction + "* position.\n"
                ...
                log.info("Watch alert sent: " + label + " | regime_flip=" + str(regime_flip) + " | quality=" + str(quality_score))
