...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Pattern 1: Sweep status ───────────────────────────────────────────
        if _sweep_status:
            msg += f"💧 {_sweep_status}\n"

        # ── Pattern 2: POI density ────────────────────────────────────────────
        _poi_icon = "🟢" if _poi["density"] >= 3 else ("🟡" if _poi["density"] == 2 else "🔴")
        if _poi["density"] > 0:
            msg += f"{_poi_icon} Zone: {_poi['label']} ({_poi['density']} layers: {', '.join(_poi['layers'])})\n"
        else:
            msg += f"{_poi_icon} Zone: Single level — limited structural confluence\n"
        ...
        # ── Position sizing ───────────────────────────────────────────────────
        try:
            if sl_price and sl_price > 0 and _pullback_entry > 0:
        ...
        # ── Inline limit plan (Pattern 1+2 combined) — only when WAIT ────────
        # When refine says WAIT, the pullback zone is a future limit entry.
        # Show the zone quality and set-and-forget command inline instead of
        # requiring user to run /limit separately.
        _pb_target = entry_check.get("pullback_target")
        if action in ("WAIT", "CONDITIONAL") and _pb_target and _pb_target > 0:
