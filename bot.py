...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Format message ─────────────────────────────────────────────────
        def _esc_r(s):
            """Strip chars that break Telegram Markdown parser."""
        ...
        if grade_result:
            ...
            msg += f"{grade_icon} *Grade {grade_result['grade']}* | Dow: {_esc_r(grade_result.get('dow_phase','UNCLEAR'))}\n"
            if grade_result.get("reasons"):
                msg += "✔ " + _esc_r(" | ".join(grade_result["reasons"][:3])) + "\n"
            if grade_result.get("warnings"):
                msg += "⚠ " + _esc_r(" | ".join(grade_result["warnings"][:2])) + "\n"
        if acc_result["is_accumulating"]:
            msg += f"\n📦 *Accumulation* ({_esc_r(acc_result['confidence'])}, {acc_result['score']}/7)\n"
            if acc_result["signals"]:
                msg += _esc_r(acc_result['signals'][0]) + "\n"
        if sk_result["is_shakeout"]:
            msg += f"\n⚡ *Shakeout* ({_esc_r(sk_result['confidence'])}, {sk_result['score']}/8)\n"
            msg += f"Flush: ${sk_result['flush_low']:.4f} | Target: ${sk_result['pump_target']:.4f}\n"
            if sk_result["signals"]:
                msg += _esc_r(sk_result['signals'][0]) + "\n"
        if sl_price:
...
async def cmd_paper_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg += _esc(entry_check['reason']) + "\n"
        if entry_check.get("pullback_target"):
            msg += f"Pullback target: ${entry_check['pullback_target']:.4f}\n"
        msg += f"\n{grade_icon} *Grade {grade_str}* | Dow: {_esc(dow_phase)}\n"
        if grade_result and grade_result.get("reasons"):
            msg += "✔ " + _esc(" | ".join(grade_result["reasons"][:2])) + "\n"
        if grade_result and grade_result.get("warnings"):
            msg += "⚠ " + _esc(" | ".join(grade_result["warnings"][:1])) + "\n"
        if sk["is_shakeout"]:
            msg += f"\n⚡ *Shakeout* ({_esc(sk['confidence'])}, {sk['score']}/8)\n"
            msg += f"Flush: ${sk['flush_low']:.4f} | Target: ${sk['pump_target']:.4f}\n"
        if acc["is_accumulating"]:
            msg += f"\n📦 *Accumulation* ({_esc(acc['confidence'])}, {acc['score']}/7)\n"
        msg += f"\n📌 SL: ${sl:.4f} ({sl_pct:.1f}%)"
        msg += f" | TP1: ${tp1:.4f} ({tp1_pct:.1f}%)"
        msg += f" | TP2: ${tp2:.4f} ({tp2_pct:.1f}%)\n"
        msg += f"💥 Liquidation at {leverage}x: ${liq:.4f}\n"
        msg += "\nTrack with /portfolio — close analysis with /paper_stats"
