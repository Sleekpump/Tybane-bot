...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── 1. Conviction tier ────────────────────────────────────────────────
        if action == "ENTER":
            _score_for_tier = _real_score if "_real_score" in locals() else 60.0
        ...
        if "_sq" in locals() and _sq:
            ...
            if _gs:
                def _grp_line(name, val, icon):
                    arrow = "+" if val > 0.12 else ("-" if val < -0.12 else "~")
                    bar   = "▓▓▓" if abs(val) > 0.35 else ("▓▓" if abs(val) > 0.18 else "▓")
                    return f"{icon} {arrow}{bar} {name} ({val:+.2f})"
                ...
                _score_display = _real_score if "_real_score" in locals() else _sq.get("quality_score", 0)
        ...
        _ec_reason = entry_check.get("reason", "")
        if _ec_reason:
            msg += f"\n*Momentum:*\n{_ec_reason}\n"
