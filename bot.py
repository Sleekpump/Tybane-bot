...
rescan_watches          = {}  # {symbol: {...}} — /rescanwatch command, entry-wait monitoring only, auto_watch never touches this
_rescan_last_cycle_time = 0   # timestamp of last rescan cycle — used by /rescanstatus
...
RESCAN_INTERVAL         = 900   # re-evaluate watched coins every 15 min
RESCAN_COOLDOWN         = 3600  # 60 min between rescan alerts per coin
RESCAN_QUALITY_MIN      = 55    # minimum quality score to send a rescan alert
RESCAN_MAX_ATR_PCT      = 5.0   # skip coin if ATR > 5% of price (still erratic)
RESCAN_PROXIMITY        = 15.0  # skip coin if price > 15% from watch zone
RESCAN_WATCH_MAX        = 10    # maximum coins in rescan_watches at once
...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Fetch funding FIRST — needed by compute_signal_quality for exhaustion ──
        funding_rate     = 0.0   # raw Bitget scale
        funding_rate_pct = 0.0   # display only
        funding_warn     = ""
        try:
            _fr = exchange.fetch_funding_rate(symbol)
            funding_rate     = float(_fr.get("fundingRate", 0.0))
        ...
        # Funding already fetched above before _sq — no second fetch needed
        ...
        try:
            ...
            for symbol, sig in list(active_signals.items()):
                try:
                    ...
                    # ── Stage 1: Before TP1 — original SL, no trailing ────────
                    if not sig["tp1_hit"]:
                        if (direction == "LONG" and price >= tp1) or (direction == "SHORT" and price <= tp1):
                            # ── TP1 Hit ───────────────────────────────────────────────
                            # Stage 2: Partial validation.
                            # Do NOT move to breakeven — move SL to structural pivot.
                            # Get last confirmed swing low (LONG) or swing high (SHORT)
                            # from 1H structure. If unavailable, fall back to buffer %.
                            _struct_sl = None
                            try:
                                # Use MEXC if this is a MEXC-only coin
                                _is_mexc_coin = (active_signals.get(symbol, {}).get("use_mexc")
                                                 or watched_trades.get(symbol, {}).get("use_mexc"))
                                _df_sl = (fetch_ohlcv_mexc(symbol, "1h", 30)
                                          if _is_mexc_coin else fetch_ohlcv(symbol, "1h", 30))
                                if _df_sl is not None and len(_df_sl) >= 10:
                                    if direction == "LONG":
                                        # Last confirmed higher low — look for lowest low
                                        # in bars -20 to -3 (exclude recent forming candles)
                                        _recent_lows  = _df_sl["low"].iloc[-20:-3]
                                        _swing_low    = float(_recent_lows.min())
                                        # Only use if it's above entry (already in profit zone)
                                        # and below current price (gives room to breathe)
                                        if entry < _swing_low < price * 0.98:
                                            _struct_sl = round(_swing_low * 0.998, 6)
                                    else:
                                        _recent_highs = _df_sl["high"].iloc[-20:-3]
                                        _swing_high   = float(_recent_highs.max())
                                        if entry > _swing_high > price * 1.02:
                                            _struct_sl = round(_swing_high * 1.002, 6)
                            except Exception:
                                pass
                            ...
                            # Trailing stop: use structural level if found, else buffer %
                            trailing_stop = (_struct_sl if _struct_sl
                                             else (round(price * (1 - _tbuf), 6)
                                                   if direction == "LONG"
                                                   else round(price * (1 + _tbuf), 6)))

                            # BOS gate: trail_armed = False until new BOS confirms post-TP1.
                            # Scalps skip BOS gate (resolve too fast for structure to form).
                            _is_scalp_sig = sig.get("trade_type") == "scalp"
                            update_trade_state(symbol, {
                                "tp1_hit":          True,
                                "trailing_stop":    trailing_stop,
                                "trailing_extreme": price,
                                "trail_armed":      _is_scalp_sig,  # scalp: armed immediately
                                "trail_bos_candles": 0,             # counts BOS candles post-TP1
                            })
                            _tbuf_pct = int(_tbuf * 100)

                            # Message: structural SL recommendation, not breakeven
                            msg  = "\U0001f3af *TP1 Hit: " + label + "*\n\n"
                            msg += "Price: $" + "{:.4f}".format(price) + "\n"
                            msg += "Action: Close 50% of your position\n"
                            if _struct_sl:
                                msg += "Move SL to: `$" + "{:.4f}".format(_struct_sl) + "` (last swing " + ("low" if direction == "LONG" else "high") + " — structural pivot)\n"
                            else:
                                msg += "Move SL to: `$" + "{:.4f}".format(trailing_stop) + "` (" + str(_tbuf_pct) + "% buffer — no clear pivot)\n"
                            msg += "\U00002705 Trailing active" if _is_scalp_sig else "\u23f3 Trailing arms after BOS continuation confirms"
                            await send_msg(app, msg)

                    # ── Stage 2/3: After TP1 — trailing management ────────────────
                    elif sig["tp1_hit"] and sig.get("trailing_stop"):
                        trailing_stop    = sig["trailing_stop"]
                        trailing_extreme = sig["trailing_extreme"]
                        trail_armed      = sig.get("trail_armed", True)  # default True for old signals

                        # Single 1H fetch — reused by BOS gate, vol gate, and structural trail.
                        # Uses MEXC if coin is not on Bitget (_use_mexc auto-detected above).
                        _df_1h_trail = None
                        try:
                            _df_1h_trail = (fetch_ohlcv_mexc(symbol, "1h", 50)
                                            if _use_mexc else fetch_ohlcv(symbol, "1h", 50))
                        except Exception:
                            pass

                        # ── BOS continuation gate (swing only) ───────────────────
                        # Wait for a new BOS candle after TP1 before ratcheting.
                        # This avoids the pattern: TP1 → wick pullback → trailing hit → trend resumes.
                        if not trail_armed and sig.get("trade_type") != "scalp":
                            try:
                                _df_bos_arm = _df_1h_trail
                                if _df_bos_arm is not None and len(_df_bos_arm) >= 10:
                                    # Simple BOS proxy: last 3 candles all closed in trend direction
                                    # without a lower low (LONG) or higher high (SHORT)
                                    _c = _df_bos_arm["close"].values
                                    _l = _df_bos_arm["low"].values
                                    _h = _df_bos_arm["high"].values
                                    if direction == "LONG":
                                        # New higher low formed: last low > low 3 bars ago
                                        _new_hl = float(_l[-2]) > float(_l[-5])
                                        # Price above TP1 still
                                        _above_tp1 = float(_c[-1]) > tp1
                                        if _new_hl and _above_tp1:
                                            update_trade_state(symbol, {"trail_armed": True})
                                            trail_armed = True
                                            log.info(f"Trail armed (BOS confirmed): {label}")
                                    else:
                                        _new_lh = float(_h[-2]) < float(_h[-5])
                                        _below_tp1 = float(_c[-1]) < tp1
                                        if _new_lh and _below_tp1:
                                            update_trade_state(symbol, {"trail_armed": True})
                                            trail_armed = True
                                            log.info(f"Trail armed (BOS confirmed): {label}")
                            except Exception:
                        ...
                        # ── Volatility gate ───────────────────────────────────────
                        # Do NOT ratchet trailing when ATR is in 75th+ percentile
                        # of own history. Volatile expansion phases wick hard —
                        # tight trailing gets farmed constantly during these.
                        _vol_ok = True
                        if trail_armed:
                            try:
                                _df_vol = _df_1h_trail
                                if _df_vol is not None and len(_df_vol) >= 20:
                                    _atr_s = ta.atr(_df_vol["high"], _df_vol["low"],
                                                    _df_vol["close"], length=14)
                                    if _atr_s is not None and len(_atr_s.dropna()) >= 20:
                                        _atr_vals   = _atr_s.dropna().values
                                        _atr_now    = float(_atr_vals[-1])
                                        _atr_pct_rk = float(
                                            sum(1 for v in _atr_vals if v <= _atr_now)
                                            / len(_atr_vals) * 100
                                        )
                                        if _atr_pct_rk >= 75:
                                            _vol_ok = False
                                            log.info(f"Trail ratchet paused — ATR {_atr_pct_rk:.0f}th pct: {label}")
                            except Exception:
                                pass

                        # ── Ratchet trailing — only when armed and volatility OK ──
                        _tbuf_r = (TRAILING_BUFFER_SCALP
                                   if sig.get("trade_type") == "scalp"
                                   else TRAILING_BUFFER)
                        if trail_armed and _vol_ok:
                            if direction == "LONG" and price > trailing_extreme:
                                new_extreme = price
                                # Stage 3: use structural swing low when possible
                                try:
                                    _df_str = _df_1h_trail
                                    if _df_str is not None and len(_df_str) >= 10:
                                        _sw_low = float(_df_str["low"].iloc[-10:-2].min())
                                        new_stop = (round(_sw_low * 0.998, 6)
                                                    if _sw_low > trailing_stop and _sw_low < price * 0.98
                                                    else round(price * (1 - _tbuf_r), 6))
                                    else:
                                ...
                                try:
                                    _df_str = _df_1h_trail
                                    if _df_str is not None and len(_df_str) >= 10:
                                        _sw_high = float(_df_str["high"].iloc[-10:-2].max())
                                        new_stop = (round(_sw_high * 1.002, 6)
                                                    if _sw_high < trailing_stop and _sw_high > price * 1.02
                                                    else round(price * (1 + _tbuf_r), 6))
                                    else:
                                        new_stop = round(price * (1 + _tbuf_r), 6)
                                except Exception:
...
async def _run_counter_scan(app, triggered_by_command=False):
    ...
    for symbol, label in list(COIN_LABELS.items()):
        ...
        try:
            ...
            if change_4h >= 5.0 or change_1h >= 4.0:
                ...
                # ── Coin regime filter ────────────────────────────────────────
                # Counter-scalp SHORTs only valid if coin is TRENDING_DOWN or
                # RANGING on 4H. Shorting a TRENDING_UP coin (SPACE/INTC case —
                # ADX 35, price above SMA) means fighting a confirmed uptrend.
                # Best counter-scalp SHORTs are on coins with their OWN downtrend
                # — the bounce is a counter-move within the macro direction.
                _coin_adx     = 20.0
                _coin_regime  = "RANGING"
                try:
                    _adx_df = ta.adx(df_4h["high"], df_4h["low"], df_4h["close"], length=14)
                    if _adx_df is not None and "ADX_14" in _adx_df.columns:
                        _coin_adx = float(_adx_df["ADX_14"].dropna().iloc[-1])
                    _sma20_val = float(df_4h["close"].rolling(20).mean().iloc[-1])
                    _coin_regime = (
                        "TRENDING_UP"   if _coin_adx >= 28 and price_now > _sma20_val else
                        "TRENDING_DOWN" if _coin_adx >= 28 and price_now < _sma20_val else
                        "RANGING"
                    )
                except Exception:
                    pass  # safe default: RANGING — allow through

                if _coin_regime == "TRENDING_UP":
                    log.info(
                        f"Counter scan: {label} SHORT blocked — coin TRENDING_UP "
                        f"(ADX {_coin_adx:.1f}), counter-scalp opposes own trend"
                    )
                    continue
                ...
                _cs_rr_min = 2.0 if btc_circuit_breaker.get("tier") == "full" else 1.5
                if c_rr < _cs_rr_min:
                    continue
                ...
                # ── Funding rate ──────────────────────────────────────────────
                # Positive funding = longs paying shorts = longs overcrowded
                # → SHORT counter-scalp is stronger (crowd caught wrong side)
                _cs_funding_pct = 0.0
                try:
                    _cs_fr = exchange.fetch_funding_rate(symbol)
                    _cs_funding_pct = float(_cs_fr.get("fundingRate", 0.0)) * 100
                except Exception:
                    pass

                # ── Conviction score (0–100) ──────────────────────────────────
                # Composite of R:R, zone density, volume, funding alignment.
                # Used for ranking — best setups surface first.
                _cs_density    = _poi.get("density", 0)
                _cs_rr_score   = min(30, max(0, (c_rr - 1.5) * 20))
                _cs_den_score  = {3: 30, 2: 15}.get(min(_cs_density, 3), 0)
                _cs_vol_score  = max(0, round(20 - (_vol_ratio - 0.5) * 13.3))
                _cs_fund_score = (15 if _cs_funding_pct > 0.05
                                  else 5 if abs(_cs_funding_pct) <= 0.05
                                  else 0)
                # Regime alignment bonus — SHORT with coin TRENDING_DOWN is
                # trading WITH the micro trend, not against it.
                _cs_regime_score = (10 if _coin_regime == "TRENDING_DOWN"
                                    else 5 if _coin_regime == "RANGING"
                                    else 0)
                _cs_conviction = min(100, round(_cs_rr_score + _cs_den_score + _cs_vol_score + _cs_fund_score + _cs_regime_score))
                ...
                alerts.append({
                    ...
                    "decoupled":   False,
                    "vol_ratio":   round(_vol_ratio, 1),
                    "rsi_val":     round(_rsi_v, 1),
                    "funding_pct": round(_cs_funding_pct, 4),
                    "conviction":  _cs_conviction,
                    "coin_regime": _coin_regime,
                    "coin_adx":    round(_coin_adx, 1),
                })
                found += 1

            # ── Check for SHORT→LONG counter (coin dumped, bounce back to OB) ─
            elif change_4h <= -5.0 or change_1h <= -4.0:
                # Adaptive: suppress LONG counter-scalps when BTC is FULL BLOCK.
                # Bouncing long against a sustained macro downtrend is high risk.
                if btc_circuit_breaker.get("tier") == "full":
                    log.info(f"Counter scan: {label} LONG counter skipped — BTC FULL BLOCK")
                    continue
                ob_data = detect_order_blocks(df_1h)
                ...
                # ── Coin regime filter ────────────────────────────────────────
                # Counter-scalp LONGs only valid if coin is TRENDING_UP or
                # RANGING on 4H. Buying a bounce on a TRENDING_DOWN coin means
                # fighting a confirmed downtrend. Best counter-scalp LONGs are
                # decoupled coins or coins in their own uptrend (BSB type).
                _coin_adx     = 20.0
                _coin_regime  = "RANGING"
                try:
                    _adx_df = ta.adx(df_4h["high"], df_4h["low"], df_4h["close"], length=14)
                ...
                if _coin_regime == "TRENDING_DOWN":
                    log.info(
                        f"Counter scan: {label} LONG blocked — coin TRENDING_DOWN "
                        f"(ADX {_coin_adx:.1f}), counter-scalp LONG opposes own trend"
                    )
                    continue
                try:
                    _atr_s   = ta.atr(df_1h["high"], df_1h["low"], df_1h["close"], length=14)
                ...
                # ── Funding rate ──────────────────────────────────────────────
                # Negative funding = shorts paying longs = shorts overcrowded
                # → LONG counter-scalp is stronger (crowd caught wrong side)
                _cs_funding_pct = 0.0
                try:
                    _cs_fr = exchange.fetch_funding_rate(symbol)
                    _cs_funding_pct = float(_cs_fr.get("fundingRate", 0.0)) * 100
                ...
                _cs_decouple      = 10 if _is_decoupled else 0
                # Regime alignment bonus — LONG with coin TRENDING_UP is
                # trading WITH the micro trend. Decoupled coins get this too.
                _cs_regime_score  = (10 if _coin_regime == "TRENDING_UP" or _is_decoupled
                                     else 5 if _coin_regime == "RANGING"
                                     else 0)
                _cs_conviction = min(100, round(_cs_rr_score + _cs_den_score + _cs_vol_score + _cs_fund_score + _cs_decouple + _cs_regime_score))
    ...
    # Sort by conviction score descending — best composite setups first
    ...
    for a in alerts[:5]:   # cap at 5 alerts per scan
        ...
        conv_bar      = "🟢" if conviction >= 65 else "🟡" if conviction >= 40 else "🔴"
        funding_pct   = a.get("funding_pct", 0.0)
        vol_ratio     = a.get("vol_ratio", 1.0)
        rsi_val       = a.get("rsi_val", 50.0)
        coin_regime   = a.get("coin_regime", "RANGING")
        coin_adx      = a.get("coin_adx", 0.0)
        regime_e      = ("📈" if coin_regime == "TRENDING_UP"
                         else "📉" if coin_regime == "TRENDING_DOWN"
                         else "↔️")
        # Funding label
        if a["direction"] == "SHORT":
            fund_note = (f"🔥 +{funding_pct:.3f}% (longs paying — aligned)"
                         if funding_pct > 0.05
                         else f"⚠ {funding_pct:+.3f}% (shorts paying — misaligned)"
                         if funding_pct < -0.05
                         else f"{funding_pct:+.3f}% (neutral)")
        else:
            fund_note = (f"🔥 {funding_pct:.3f}% (shorts paying — aligned)"
                         if funding_pct < -0.05
                         else f"⚠ +{funding_pct:.3f}% (longs paying — misaligned)"
                         if funding_pct > 0.05
                         else f"{funding_pct:+.3f}% (neutral)")
        msg += f"{'—' * 20}\n"
        ...
        msg += f"{conv_bar} Conviction: {conviction}/100 | RSI: {rsi_val:.0f} | Vol: {vol_ratio:.1f}x avg\n"
        msg += f"{regime_e} Coin regime: {coin_regime} (ADX {coin_adx:.1f})\n"
        msg += f"Funding: {fund_note}\n"
        _cs_tier = btc_circuit_breaker.get("tier", "none")
        if a["base_dir"] == "LONG" and _cs_tier == "full":
            msg += f"⛔ Do not flip LONG — BTC circuit breaker FULL BLOCK\n"
        elif a["base_dir"] == "LONG" and _cs_tier == "elevated":
            msg += f"⚠ Flip to {a['base_dir']} from zone — BTC elevated, higher risk\n"
        else:
...
async def cmd_rescanwatch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    /rescanwatch — standalone entry-wait monitor. No /watch required.

    Adds a coin to rescan_watches (separate from watched_trades — auto_watch
    never touches this dict). The rescan loop re-evaluates every 15 minutes
    and alerts when quality, grade and ATR conditions improve.

    Usage:
      /rescanwatch COIN DIRECTION PRICE [trade_type] [leverage]
      /rescanwatch COIN off
      /rescanwatch          — list all currently rescan-watched coins

    Examples:
      /rescanwatch ONDO SHORT 0.3942
      /rescanwatch ONDO SHORT 0.3942 swing 10
      /rescanwatch ONDO off
    """
    args = ctx.args

    # ── No args — list current rescan watches ────────────────────────────────
    if not args:
        if not rescan_watches:
            await update.message.reply_text(
                "🔄 No coins in rescan watch.\n\n"
                "Add one: /rescanwatch ONDO SHORT 0.3942"
            )
            return
        msg = "🔄 *Rescan Watch List:*\n\n"
        for _sym, _rw in rescan_watches.items():
            _rw_last = _rw.get("last_rescan_alert", 0)
            _rw_ago  = f"{round((_time.time() - _rw_last) / 60)}m ago" if _rw_last else "never"
            msg += (f"• *{_rw['label']}* {_rw['direction']} "
                    f"{_rw.get('trade_type','swing').upper()} "
                    f"@ `${_rw['entry']:.4f}` | "
                    f"{_rw.get('leverage', 10)}x | last alert: {_rw_ago}\n")
        try:
            await update.message.reply_text(msg, parse_mode="Markdown")
        except Exception:
            await update.message.reply_text(msg, parse_mode=None)
    ...
    # ── Off — remove from rescan watches ─────────────────────────────────────
    if len(args) == 2 and args[1].lower() == "off":
        symbol = coin + "/USDT:USDT"
        if symbol in rescan_watches:
            del rescan_watches[symbol]
            await update.message.reply_text(
                f"🔄 Rescan monitoring stopped for *{coin}*.", parse_mode="Markdown"
            )
        else:
            await update.message.reply_text(f"⚠ {coin} is not in the rescan watch list.")
        return

    # ── Add — parse direction and price ──────────────────────────────────────
    if len(args) < 3:
        await update.message.reply_text(
            "Usage: /rescanwatch COIN DIRECTION PRICE [trade_type] [leverage]\n"
            "Example: /rescanwatch ONDO SHORT 0.3942 swing 10"
        )
        return
    ...
    try:
        ...
        await update.message.reply_text(
            "⚠ Price must be a number. Example: /rescanwatch ONDO SHORT 0.3942"
        )
        return

    trade_type = args[3].lower() if len(args) > 3 else "swing"
    try:
        leverage = int(args[4]) if len(args) > 4 else 10
    except ValueError:
        leverage = 10
    symbol     = coin + "/USDT:USDT"

    if symbol not in rescan_watches and len(rescan_watches) >= RESCAN_WATCH_MAX:
        await update.message.reply_text(
            f"⚠ Rescan watch is full ({RESCAN_WATCH_MAX} coins max).\n"
            f"Remove one first: /rescanwatch COIN off"
        )
        return

    rescan_watches[symbol] = {
        "label":             coin,
        "direction":         direction,
        "entry":             entry_price,
        "trade_type":        trade_type,
        "leverage":          leverage,
        "quality_min":       RESCAN_QUALITY_MIN,
        "added_at":          _time.time(),
        "last_rescan_alert": 0,
    }
    ...
    await update.message.reply_text(
        f"🔄 *Rescan watch added: {coin}*\n\n"
        f"{dir_e} *{direction}* | {trade_type.upper()} | "
        f"Zone: `${entry_price:.4f}` | {leverage}x\n\n"
        f"Alerts when:\n"
        f"• Quality ≥ {RESCAN_QUALITY_MIN}/100\n"
        f"• Grade A or B\n"
        f"• ATR ≤ {RESCAN_MAX_ATR_PCT}%\n"
        f"• Direction confirmed\n\n"
        f"_/rescanwatch {coin} off to stop_",
        parse_mode="Markdown"
    )


async def cmd_rescanstatus(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    /rescanstatus — show exactly which gate is blocking each rescan-watched coin.

    Reads from the cache written by auto_rescan_watched after each cycle.
    No pipeline is run — this is instant and free.
    """
    if not rescan_watches:
        await update.message.reply_text(
            "🔄 No coins in rescan watch.\n\nAdd one: /rescanwatch COIN DIRECTION PRICE"
        )
        return
    ...
    # ── Cycle timing header ───────────────────────────────────────────────────
    if _rescan_last_cycle_time > 0:
        _rs_since = round((now - _rescan_last_cycle_time) / 60)
        _rs_next  = max(0, round((RESCAN_INTERVAL - (now - _rescan_last_cycle_time)) / 60))
        cycle_str = f"Last cycle: {_rs_since}m ago | Next: ~{_rs_next}m"
    else:
        cycle_str = "First cycle not yet run (90s after boot)"

    msg = f"🔄 *Rescan Status*\n_{cycle_str}_\n\n"

    for _rs_sym, _rs_w in rescan_watches.items():
        _rs_label     = _rs_w.get("label", _rs_sym.split("/")[0])
        _rs_direction = _rs_w.get("direction", "?")
        _rs_entry     = _rs_w.get("entry", 0)
        _rs_type      = _rs_w.get("trade_type", "swing").upper()
        _rs_dir_e     = "🟢" if _rs_direction == "LONG" else "🔴"

        # Last check timing
        _rs_checked = _rs_w.get("_last_checked", 0)
        _rs_checked_str = f"{round((now - _rs_checked) / 60)}m ago" if _rs_checked else "never"

        # Cooldown status
        _rs_last_alert = _rs_w.get("last_rescan_alert", 0)
        _rs_cd_left    = RESCAN_COOLDOWN - (now - _rs_last_alert)
        _rs_cd_str     = f"🕐 Gate 1 — Cooldown: {round(_rs_cd_left / 60)}m remaining" if _rs_cd_left > 0 else ""

        # Cached pipeline result
        _rs_quality  = _rs_w.get("_last_quality")
        _rs_grade    = _rs_w.get("_last_grade")
        _rs_det_dir  = _rs_w.get("_last_direction")
        _rs_atr      = _rs_w.get("_last_atr_pct")
        _rs_blocked  = _rs_w.get("_blocked_at", "Awaiting first scan cycle")

        msg += f"{_rs_dir_e} *{_rs_label} {_rs_direction}* {_rs_type} @ `${_rs_entry:.4f}`\n"
        msg += f"Last checked: {_rs_checked_str}\n"

        if _rs_cd_str:
            msg += f"{_rs_cd_str}\n"
        elif _rs_quality is not None:
            _rs_g_e = {"A": "🟣", "B": "🟢", "C": "🟡"}.get(_rs_grade, "⚪")
            msg += (f"Pipeline: Q={_rs_quality} | {_rs_g_e}G={_rs_grade} | "
                    f"Dir={_rs_det_dir} | ATR={_rs_atr:.1f}%\n")
            msg += f"❌ *{_rs_blocked}*\n" if not _rs_blocked.startswith("✅") else f"*{_rs_blocked}*\n"
        else:
            msg += f"⏳ {_rs_blocked}\n"
            ...
            try:
                ...
                if not _use_mexc:
                    try:
                        ...
                        # Persist so the flag survives Railway restarts
                        try:
                            save_json(WATCH_FILE, {
                                k: {f: v for f, v in w.items()
                                    if f not in ("last_alert", "last_early_alert", "last_bos_alert")}
                                for k, w in watched_trades.items()
                            })
                        except Exception:
                            pass
...
# ─── WATCH RESCAN LOOP ─────────────────────────────────────────────────────────
def _build_rescan_alert(label, direction, grade, quality, confidence,
                        price, watch_price, atr_pct, leverage, trade_type):
    """Format the Telegram alert message when a watched coin clears all rescan gates."""
    dir_e   = "🟢" if direction == "LONG" else "🔴"
    grade_e = {"A": "🟣", "B": "🟢"}.get(grade, "⚪")
    conf_e  = {"HIGH": "🔥 ", "MEDIUM": "⚡ "}.get(confidence, "")
    # Always use live pipeline price for the refine command — not the stale
    # watch zone price the user typed. By the time the alert fires, price may
    # have moved significantly from the original zone.
    refine_cmd = f"/refine {label} {direction} {price:.4f} {leverage} {trade_type}"
    price_str  = f"${price:.4f}" if price > 0 else "unknown"
    zone_str   = f"${watch_price:.4f}" if watch_price else "not set"
    # Show drift between watch zone and current price so user can judge relevance
    drift_str  = ""
    if watch_price and watch_price > 0 and price > 0:
        _drift = (price - watch_price) / watch_price * 100
        drift_str = f" ({_drift:+.1f}% from zone)"
    msg  =  f"🔔 *Watch Rescan Alert: {label}*\n\n"
    msg +=  f"Conditions improved — entry window may be open.\n\n"
    msg +=  f"{dir_e} *{direction}* | {grade_e} Grade *{grade}* | {conf_e}Q: *{quality}/100*\n"
    msg +=  f"Current price: `{price_str}`{drift_str}\n"
    msg +=  f"Watch zone:    `{zone_str}`\n"
    msg +=  f"ATR: {atr_pct:.1f}% ✅  |  Confidence: {confidence}\n\n"
    msg +=  f"*Confirm entry at current price:*\n"
    msg +=  f"`{refine_cmd}`\n\n"
    msg +=  f"_/rescanwatch {label} off to stop monitoring_"
    return msg


def _get_rescan_adaptive_params(direction):
    """
    Return adaptive gate parameters for the rescan loop based on current
    BTC circuit breaker state and market regime.

    Reads two existing globals — btc_circuit_breaker and _btc_market_regime_cache —
    and returns thresholds calibrated to market conditions. No side effects.

    Returns dict with:
      long_blocked      — True when FULL BLOCK (skip LONG pipeline entirely)
      quality_min       — adjusted quality threshold (replaces RESCAN_QUALITY_MIN)
      neutral_soft_pass — True when NEUTRAL direction is treated as a soft pass
      neutral_quality   — quality needed when NEUTRAL soft-passes (compensates leniency)
      btc_tier          — current tier string for notification tracking
      regime            — current regime string for logging
    """
    btc_tier = btc_circuit_breaker.get("tier", "none")   # "none" | "elevated" | "full"
    regime   = _btc_market_regime_cache.get("regime", "RANGING")

    long_blocked      = False
    quality_min       = RESCAN_QUALITY_MIN  # default 55
    neutral_soft_pass = False
    neutral_quality   = 60                  # compensatory threshold when NEUTRAL soft-passes

    if btc_tier == "full":
        if direction == "LONG":
            long_blocked = True             # suspend entirely — no pipeline call
        else:
            # SHORT aligns with sustained downtrend — relax gates
            quality_min       = 45
            neutral_soft_pass = True
            neutral_quality   = 60

    elif btc_tier == "elevated":
        if direction == "LONG":
            quality_min = 65                # weakening macro — need stronger LONG evidence
        else:
            quality_min       = 45
            neutral_soft_pass = True
            neutral_quality   = 60

    else:                                   # BTC clear — use regime to calibrate
        if regime == "TRENDING_DOWN":
            quality_min = 45 if direction == "SHORT" else 65
        elif regime == "TRENDING_UP":
            quality_min = 45 if direction == "LONG"  else 65
        else:                               # RANGING — pipeline is naturally conservative
            quality_min = 45

    return {
        "long_blocked":      long_blocked,
        "quality_min":       quality_min,
        "neutral_soft_pass": neutral_soft_pass,
...
async def auto_rescan_watched(app):
    """
    Background loop — re-evaluates every watched coin every 15 minutes.

    Gates (all must pass before an alert fires):
      1. Cooldown        — 60 min minimum between rescan alerts per coin
      2. Proximity       — price must be within 15% of the watch zone
      3. Full pipeline   — same analyze_async() used by /coin and /scan
      4. Direction match — detected direction must equal watched direction
      5. Quality gate    — score must reach per-watch quality_min (default 55)
      6. Grade gate      — Grade C always blocked (A or B required)
      7. ATR gate        — ATR must be ≤ 5% of price (erratic coins skipped)

    On pass: sends alert with pre-filled /refine command.
    """
    global _rescan_last_cycle_time
    log.info("Watch rescan loop started")
    await asyncio.sleep(90)  # startup offset — let BTC monitor and coin refresh init first
    ...
    while True:
        ...
        _rescan_last_cycle_time = _time.time()
        log.info(f"Rescan cycle: evaluating {len(rescan_watches)} rescan-watched coin(s)")

        for symbol, w in list(rescan_watches.items()):
            try:
                now         = _time.time()
                label       = w.get("label", symbol.split("/")[0])
                direction   = w.get("direction", "LONG")
                watch_price = w.get("entry")
                last_rescan = w.get("last_rescan_alert", 0)

                # ── Adaptive params — read BTC state + regime once per coin ──
                _rsc_p        = _get_rescan_adaptive_params(direction)
                _rsc_btc_tier = _rsc_p["btc_tier"]
                _rsc_prev_tier = w.get("_last_btc_tier", "none")

                # ── LONG block gate — suspend LONGs during BTC FULL BLOCK ─────
                # Exception: if the coin is moving UP 3%+ on 1H while BTC is
                # moving DOWN, it is decoupled from macro — let it through.
                # BSB-type bounces off extreme demand zones fall into this category.
                if _rsc_p["long_blocked"]:
                    _rsc_decoupled = False
                    try:
                        _rsc_coin_1h = fetch_ohlcv(symbol, "1h", 5)
                        _rsc_btc_1h  = fetch_ohlcv("BTC/USDT:USDT", "1h", 5)
                        if (_rsc_coin_1h is not None and len(_rsc_coin_1h) >= 5
                                and _rsc_btc_1h is not None and len(_rsc_btc_1h) >= 5):
                            _rsc_coin_chg = (
                                (float(_rsc_coin_1h["close"].iloc[-1]) -
                                 float(_rsc_coin_1h["close"].iloc[-4])) /
                                float(_rsc_coin_1h["close"].iloc[-4]) * 100
                            )
                            _rsc_btc_chg  = (
                                (float(_rsc_btc_1h["close"].iloc[-1]) -
                                 float(_rsc_btc_1h["close"].iloc[-4])) /
                                float(_rsc_btc_1h["close"].iloc[-4]) * 100
                            )
                            if _rsc_coin_chg >= 3.0 and _rsc_btc_chg < 0:
                                _rsc_decoupled = True
                                log.info(
                                    f"Rescan {label}: LONG decoupled from BTC "
                                    f"(coin +{_rsc_coin_chg:.1f}% vs BTC {_rsc_btc_chg:.1f}%) "
                                    f"— allowing through FULL BLOCK"
                                )
                    except Exception:
                        pass  # fetch failed — be conservative, block

                    if not _rsc_decoupled:
                        if _rsc_prev_tier != "full":
                            # First cycle seeing FULL BLOCK — notify once
                            rescan_watches[symbol]["_last_btc_tier"] = "full"
                            rescan_watches[symbol]["_last_checked"]  = now
                            rescan_watches[symbol]["_blocked_at"]    = (
                                "LONG suspended — BTC FULL BLOCK active (no decoupling)"
                            )
                            try:
                                await send_msg(app,
                                    f"⛔ *Rescan: {label} LONG suspended*\n\n"
                                    f"BTC circuit breaker is FULL BLOCK.\n"
                                    f"Pipeline calls paused — monitoring resumes if:\n"
                                    f"• {label} moves up 3%+ on 1H while BTC moves down\n"
                                    f"• BTC circuit breaker clears\n\n"
                                    f"_/rescanstatus to check all watches_"
                                )
                            except Exception:
                                pass
                        continue
                    # Decoupled — fall through to full pipeline analysis
                    rescan_watches[symbol]["_blocked_at"] = (
                        f"⚡ Decoupled from BTC — LONG proceeding despite FULL BLOCK"
                    )

                # ── Resume notification — FULL BLOCK just cleared ─────────────
                if _rsc_prev_tier == "full" and _rsc_btc_tier != "full":
                    rescan_watches[symbol]["_last_btc_tier"] = _rsc_btc_tier
                    try:
                        await send_msg(app,
                            f"✅ *Rescan: {label} LONG resumed*\n\n"
                            f"BTC circuit breaker cleared ({_rsc_btc_tier}).\n"
                            f"Monitoring resumed with adaptive thresholds."
                        )
                    except Exception:
                        pass
                else:
                ...
                # ── Gate 1: cooldown ──────────────────────────────────────────
                cd_left = RESCAN_COOLDOWN - (now - last_rescan)
                if cd_left > 0:
                    log.info(f"Rescan {label}: cooldown {round(cd_left / 60)}m remaining")
                    continue

                # ── Gate 2: proximity — skip if price too far from watch zone ─
                if watch_price and watch_price > 0:
                    try:
                        _rsc_price = fetch_price_any_exchange(symbol)
                        if _rsc_price > 0:
                            _rsc_dist = abs(_rsc_price - watch_price) / watch_price * 100
                            if _rsc_dist > RESCAN_PROXIMITY:
                                log.info(
                                    f"Rescan {label}: ${_rsc_price:.4f} is {_rsc_dist:.1f}% "
                                    f"from zone ${watch_price:.4f} — skipping"
                                )
                                rescan_watches[symbol]["_last_checked"]  = now
                                rescan_watches[symbol]["_blocked_at"]    = (
                                    f"Gate 2 — Price ${_rsc_price:.4f} is {_rsc_dist:.1f}% "
                                    f"from zone ${watch_price:.4f} (max {RESCAN_PROXIMITY}%)"
                                )
                                continue
                    except Exception:
                        pass
                ...
                log.info(
                    f"Rescan: {label} {direction} | BTC={_rsc_btc_tier} "
                    f"regime={_rsc_p['regime']} | Q_min={_rsc_p['quality_min']}"
                )
                _rsc_result = await analyze_async(symbol)

                if not _rsc_result or not _rsc_result.get("direction"):
                    log.info(f"Rescan {label}: pipeline returned no result")
                    rescan_watches[symbol]["_last_checked"] = now
                    rescan_watches[symbol]["_blocked_at"]   = "Gate 3 — Pipeline returned no result"
                    continue

                _rsc_quality  = _rsc_result.get("abs_score", 0)
                _rsc_grade    = _rsc_result.get("grade", "C")
                _rsc_det_dir  = _rsc_result.get("direction", "NEUTRAL")
                _rsc_price    = _rsc_result.get("price", 0)
                _rsc_conf     = _rsc_result.get("confidence", "LOW")
                _rsc_atr      = _rsc_result.get("atr", 0)
                _rsc_atr_pct  = (_rsc_atr / _rsc_price * 100) if (_rsc_price > 0 and _rsc_atr > 0) else 0.0

                # Cache pipeline result for /rescanstatus
                rescan_watches[symbol]["_last_checked"]   = now
                rescan_watches[symbol]["_last_quality"]   = _rsc_quality
                rescan_watches[symbol]["_last_grade"]     = _rsc_grade
                rescan_watches[symbol]["_last_direction"] = _rsc_det_dir
                rescan_watches[symbol]["_last_atr_pct"]   = _rsc_atr_pct

                # Rate-limit between pipeline calls
                await asyncio.sleep(REQUEST_DELAY * 5)

                # ── Gate 4: direction match (NEUTRAL = soft pass when adaptive) ─
                if _rsc_det_dir != direction:
                    if _rsc_det_dir == "NEUTRAL" and _rsc_p["neutral_soft_pass"]:
                        # NEUTRAL is not actively opposing — allow through, but
                        # raise the quality bar to compensate for the leniency.
                        _rsc_p["quality_min"] = max(
                            _rsc_p["quality_min"], _rsc_p["neutral_quality"]
                        )
                        log.info(
                            f"Rescan {label}: NEUTRAL soft-pass "
                            f"(Q threshold raised to {_rsc_p['quality_min']})"
                        )
                        rescan_watches[symbol]["_blocked_at"] = (
                            f"Gate 4 soft-pass — NEUTRAL (not opposing {direction}), "
                            f"quality raised to {_rsc_p['quality_min']}"
                        )
                        # Do NOT continue — fall through to Gate 5 with tighter Q
                    else:
                        log.info(
                            f"Rescan {label}: pipeline→{_rsc_det_dir} | "
                            f"watching→{direction} — hard mismatch"
                        )
                        rescan_watches[symbol]["_blocked_at"] = (
                            f"Gate 4 — Direction mismatch: pipeline says "
                            f"{_rsc_det_dir}, watching {direction}"
                        )
                        continue

                # ── Gate 5: adaptive quality threshold ────────────────────────
                _rsc_q_min = _rsc_p["quality_min"]
                if _rsc_quality < _rsc_q_min:
                    log.info(f"Rescan {label}: quality {_rsc_quality} < {_rsc_q_min} — not ready")
                    rescan_watches[symbol]["_blocked_at"] = (
                        f"Gate 5 — Quality {_rsc_quality}/100 (needs ≥{_rsc_q_min} "
                        f"[adaptive: BTC={_rsc_btc_tier} regime={_rsc_p['regime']}])"
                    )
                    continue

                # ── Gate 6: grade gate (C = skip) ─────────────────────────────
                if _rsc_grade == "C":
                    log.info(f"Rescan {label}: Grade C — conditions not improved enough")
                    rescan_watches[symbol]["_blocked_at"] = (
                        "Gate 6 — Grade C (needs A or B)"
                    )
                    continue

                # ── Gate 7: ATR gate ──────────────────────────────────────────
                if _rsc_atr_pct > RESCAN_MAX_ATR_PCT:
                    log.info(
                        f"Rescan {label}: ATR {_rsc_atr_pct:.1f}% > "
                        f"{RESCAN_MAX_ATR_PCT}% — still erratic"
                    )
                    rescan_watches[symbol]["_blocked_at"] = (
                        f"Gate 7 — ATR {_rsc_atr_pct:.1f}% too high (max {RESCAN_MAX_ATR_PCT}%)"
                    )
                    continue

                # ── Gate 8: zone validity ─────────────────────────────────────
                # Checks that current price hasn't already blown through the
                # watch zone in the wrong direction — meaning the setup already
                # played out while we were waiting for conditions to improve.
                #
                # Threshold is trade_type aware:
                #   scalp → 4%  (tight targets, any meaningful overshoot = stale)
                #   swing → 8%  (wide targets, some drift from zone is acceptable)
                #
                # SHORT watch: price should still be at or above the zone.
                #   If price is threshold% BELOW zone → SHORT already played out.
                # LONG watch:  price should still be at or below the zone.
                #   If price is threshold% ABOVE zone → LONG already played out.
                if watch_price and watch_price > 0 and _rsc_price > 0:
                    _rsc_validity_pct = 4.0 if w.get("trade_type", "swing") == "scalp" else 8.0
                    _rsc_drift = (_rsc_price - watch_price) / watch_price * 100
                    _rsc_zone_blown = (
                        (direction == "SHORT" and _rsc_drift < -_rsc_validity_pct) or
                        (direction == "LONG"  and _rsc_drift >  _rsc_validity_pct)
                    )
                    if _rsc_zone_blown:
                        _rsc_blown_reason = (
                            f"price ${_rsc_price:.4f} is {abs(_rsc_drift):.1f}% "
                            f"{'below' if direction == 'SHORT' else 'above'} "
                            f"zone ${watch_price:.4f} (threshold: {_rsc_validity_pct}%)"
                        )
                        rescan_watches[symbol]["_blocked_at"] = (
                            f"Gate 8 — Zone passed: {_rsc_blown_reason}"
                        )
                        rescan_watches[symbol]["_last_checked"] = now
                        log.info(f"Rescan {label}: zone validity failed — {_rsc_blown_reason}")
                        continue

                # ── All gates passed — build and send alert ───────────────────
                rescan_watches[symbol]["_blocked_at"] = "✅ All gates passed — alert sent"
                _rsc_msg = _build_rescan_alert(
                    label       = label,
                    direction   = direction,
                    grade       = _rsc_grade,
                    quality     = _rsc_quality,
                    confidence  = _rsc_conf,
                    price       = _rsc_price,
                    watch_price = watch_price,
                    atr_pct     = _rsc_atr_pct,
                    leverage    = w.get("leverage", LEVERAGE),
                    trade_type  = w.get("trade_type", "swing"),
                )
                try:
                    await send_msg(app, _rsc_msg)
                except Exception:
                    _rsc_plain = (_rsc_msg.replace("*", "").replace("`", "").replace("_", ""))
                    await app.bot.send_message(chat_id=TELEGRAM_CHAT, text=_rsc_plain)

                rescan_watches[symbol]["last_rescan_alert"] = now
                log.info(
                    f"Rescan alert: {label} {direction} "
                    f"Q={_rsc_quality} G={_rsc_grade} ATR={_rsc_atr_pct:.1f}%"
                )

                # Small delay between alerts so Telegram isn't flooded
                await asyncio.sleep(4)

            except Exception as _rsc_err:
                log.warning(f"Rescan loop error [{symbol}]: {_rsc_err}")

        log.info("Rescan cycle complete")
...
def main():
    ...
    app.add_handler(CommandHandler("rescanwatch",   cmd_rescanwatch))
    app.add_handler(CommandHandler("rescanstatus",  cmd_rescanstatus))
    ...
    async def post_init(application):
        ...
        _tasks = [
            ...
            ("auto_rescan_watched",    auto_rescan_watched),
        ]
