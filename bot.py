                ...
                try:
                    ...
                    if not sig["tp1_hit"]:
                        if (direction == "LONG" and price >= tp1) or (direction == "SHORT" and price <= tp1):
                            trailing_stop = (round(price * (1 - TRAILING_BUFFER), 6)
                                              if direction == "LONG"
                                              else round(price * (1 + TRAILING_BUFFER), 6))
...
counter_scan_state = {}   # {symbol: last_alert_time} — 4H cooldown per coin
...
async def _run_counter_scan(app, triggered_by_command=False):
    ...
    for symbol, label in list(COIN_LABELS.items()):
        ...
        try:
            ...
            checked += 1
            price_now  = float(df_1h["close"].iloc[-1])
            price_4h_ago = float(df_4h["close"].iloc[-5])   # price 4 bars ago on 4H = 16h ago
            if price_4h_ago <= 0:
                continue
            ...
            if change_4h >= 8.0:
                ...
                # Counter SHORT: entry=now, TP=OB top, SL=recent 5-bar high + buffer
                ...
                # POI density for zone quality label
                _fvg = []
                highs = df_1h["high"].values
                lows  = df_1h["low"].values
                for _fi in range(2, len(df_1h)):
                    if lows[_fi] > highs[_fi - 2]:
                        _fvg.append({"type": "BULL", "mid": (lows[_fi] + highs[_fi - 2]) / 2,
                                     "top": lows[_fi], "bot": highs[_fi - 2]})
                    elif highs[_fi] < lows[_fi - 2]:
                        _fvg.append({"type": "BEAR", "mid": (highs[_fi] + lows[_fi - 2]) / 2,
                                     "top": lows[_fi - 2], "bot": highs[_fi]})
                _poi = detect_poi_density(ob_ref, _fvg, ob_data.get("bos_data", {}), price_now, "LONG")
                _zone_label = _poi.get("label", "OB zone")
                _zone_icon  = "🟢" if _poi.get("density", 0) >= 3 else ("🟡" if _poi.get("density", 0) >= 2 else "🔴")
            ...
            # ── Check for SHORT→LONG counter (coin dumped, bounce back to OB) ─
                ...
                _poi = detect_poi_density(ob_ref, [], ob_data.get("bos_data", {}), price_now, "SHORT")
    ...
    if not alerts:
        if triggered_by_command:
            return "⚡ *Counter Scalp Scan*\n\nNo setups found right now.\nCoins need to be 8%+ extended from their OB zone with R:R ≥ 1.5."
        return None
    ...
    for a in alerts[:5]:   # cap at 5 alerts per scan
        ...
        msg += f"{'—' * 20}\n"
        msg += f"⚡ *{a['label']} {a['direction']}* (counter scalp)\n"
                ...
                try:
                    ...
                    if not _sig.get("tp1_hit") and _tp1 > 0:
                        if (_dir == "LONG" and _px >= _tp1) or (_dir == "SHORT" and _px <= _tp1):
                            _trail = (round(_px * (1 - TRAILING_BUFFER), 6) if _dir == "LONG"
                                      else round(_px * (1 + TRAILING_BUFFER), 6))
