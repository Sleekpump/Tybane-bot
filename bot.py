...
def _build_rescan_alert(label, direction, grade, quality, confidence,
    ...
    entry_ref  = watch_price if (watch_price and watch_price > 0) else price
    refine_cmd = f"/refine {label} {direction} {entry_ref:.4f} {leverage} {trade_type}"
    ...
    msg +=  f"Current price: `{price_str}`\n"
    msg +=  f"Watch zone:    `{zone_str}`\n"
    msg +=  f"ATR: {atr_pct:.1f}% ✅  |  Confidence: {confidence}\n\n"
    msg +=  f"*Confirm entry — run now:*\n"
    msg +=  f"`{refine_cmd}`\n\n"
    msg +=  f"_/unwatch {label} to stop monitoring_"
    return msg
    ...
    while True:
        ...
        for symbol, w in list(rescan_watches.items()):
            try:
                ...
                # Sends one notification when block first activates, then goes
                # silent. Sends resume notification when block clears.
                if _rsc_p["long_blocked"]:
                    if _rsc_prev_tier != "full":
                        ...
                        rescan_watches[symbol]["_blocked_at"]    = (
                            "LONG suspended — BTC FULL BLOCK active"
                        )
                        try:
                            await send_msg(app,
                                f"⛔ *Rescan: {label} LONG suspended*\n\n"
                                f"BTC circuit breaker is FULL BLOCK.\n"
                                f"Pipeline calls paused — no LONG signals "
                                f"will be evaluated until BTC recovers.\n\n"
