...
async def cmd_rescanwatch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    /rescanwatch COIN — toggle rescan eligibility for a watched coin.

    The auto_rescan_watched loop only processes coins explicitly marked as
    rescan-eligible. This prevents it from firing on active trailing-SL trades
    that share the same watched_trades dict.

    Usage:
      /rescanwatch ONDO       — enable  rescan monitoring for ONDO
      /rescanwatch ONDO off   — disable rescan monitoring for ONDO
    """
    args = ctx.args
    if not args:
        await update.message.reply_text(
            "Usage: /rescanwatch COIN [off]\n"
            "Example: /rescanwatch ONDO\n"
            "         /rescanwatch ONDO off"
        )
        return

    coin   = args[0].upper()
    enable = len(args) < 2 or args[1].lower() != "off"
    symbol = coin + "/USDT:USDT"

    if symbol not in watched_trades:
        await update.message.reply_text(
            f"⚠ {coin} is not in your watch list.\n"
            f"Add it first with /watch."
        )
        return

    watched_trades[symbol]["rescan_eligible"] = enable
    state = "✅ enabled" if enable else "❌ disabled"
    await update.message.reply_text(
        f"🔄 Rescan monitoring {state} for *{coin}*.\n\n"
        + (
            "The rescan loop will now alert you when entry conditions improve."
            if enable else
            "The rescan loop will ignore this coin."
        ),
        parse_mode="Markdown"
    )
...
async def auto_rescan_watched(app):
    ...
    while True:
        ...
        log.info(f"Rescan cycle: evaluating {len(watched_trades)} watched coin(s)")
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                # ── Gate 0: opt-in check ──────────────────────────────────────
                # watched_trades holds BOTH entry-wait watches AND active trailing
                # SL trades. The loop must only process coins explicitly opted in
                # via /rescanwatch — never touch trailing positions automatically.
                if not w.get("rescan_eligible", False):
                    continue
                ...
                watched_trades[symbol]["last_rescan_alert"] = now
                log.info(
