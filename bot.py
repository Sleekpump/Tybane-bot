...
async def analyze_async(symbol):
    try:
        ...
        try:
            from signal_engine import check_15m_entry_quality
            df_15m = result.get("df_15m")
            if df_15m is not None and len(df_15m) >= 20 and result["direction"] not in (None, "NEUTRAL"):
                entry_check = check_15m_entry_quality(df_15m, result["direction"])
...
TRAILING_BUFFER = 0.08  # 8% trailing stop buffer after TP1 hit — raised from 6% to avoid noise hits on volatile alts
...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🔍 Analysing entry for *{raw_symbol} {direction}* @ `${entry_price}`...", parse_mode="Markdown")

    try:
        import pandas_ta as _pta
        from signal_engine import check_15m_entry_quality, grade_signal, detect_accumulation_setup

        exchange_note = ""  # default — set after fetch if MEXC used
        ...
        if df_1h is None or df_4h is None:
            await update.message.reply_text("⚠️ Could not fetch data for that symbol. Check the ticker.")
            return

        price_now      = float(df_1h["close"].iloc[-1])
        price_diff_pct = (price_now - entry_price) / entry_price * 100

        # ── IMPROVEMENT 2: Multi-timeframe RSI ───────────────────────────
        ...
        rsi_15m_v = _rsi(df_15m) if df_15m is not None and len(df_15m) >= 20 else 50.0
        rsi_1h_v  = _rsi(df_1h)
        rsi_4h_v  = _rsi(df_4h)

        # Alignment: are all TFs agreeing on direction?
        if direction == "LONG":
            rsi_aligned = rsi_15m_v < 60 and rsi_1h_v < 65 and rsi_4h_v < 70
            rsi_warning = rsi_4h_v > 70 or rsi_1h_v > 68
        else:
            rsi_aligned = rsi_15m_v > 40 and rsi_1h_v > 35 and rsi_4h_v > 30
            rsi_warning = rsi_4h_v < 30 or rsi_1h_v < 32

        # ── IMPROVEMENT 1: Live BTC context ──────────────────────────────
        btc_context = ""
        try:
            df_btc_15m, _ = fetch_ohlcv_smart("BTC/USDT:USDT", "15m", 50)
            df_btc_1h,  _ = fetch_ohlcv_smart("BTC/USDT:USDT", "1h",  50)
            if df_btc_15m is not None and df_btc_1h is not None:
                btc_rsi_15m = _rsi(df_btc_15m)
                btc_rsi_1h  = _rsi(df_btc_1h)
                btc_price   = float(df_btc_1h["close"].iloc[-1])
                # VWAP for BTC
                try:
                    btc_vwap_s = _pta.vwap(df_btc_1h["high"], df_btc_1h["low"],
                                           df_btc_1h["close"], df_btc_1h["volume"])
                    btc_vwap   = float(btc_vwap_s.dropna().iloc[-1]) if btc_vwap_s is not None and len(btc_vwap_s.dropna()) > 0 else btc_price
                    btc_vwap_dist = (btc_price - btc_vwap) / btc_vwap * 100
                    btc_vwap_str  = f"{'above' if btc_vwap_dist > 0 else 'below'} VWAP {abs(btc_vwap_dist):.1f}%"
                except Exception:
                    btc_vwap_str = ""

                # Macro verdict
                if btc_rsi_1h > 72:
                    btc_macro = "CAUTION — BTC extended, pullback risk"
                elif btc_rsi_1h < 35:
                    btc_macro = "CAUTION — BTC oversold, bounce likely but volatile"
                elif btc_rsi_1h > 55 and direction == "LONG":
                    btc_macro = "NEUTRAL — BTC trending, macro supports LONG"
                elif btc_rsi_1h < 48 and direction == "SHORT":
                    btc_macro = "NEUTRAL — BTC weakening, macro supports SHORT"
                else:
                    btc_macro = "NEUTRAL — no strong macro bias"

                btc_context = ("BTC: RSI 15m " + str(btc_rsi_15m) + " / 1H " + str(btc_rsi_1h)
                               + (" | " + btc_vwap_str if btc_vwap_str else "")
                               + "\nMacro: " + btc_macro)
        except Exception as btc_e:
            log.warning(f"Refine BTC context error: {btc_e}")

        # ── 15m entry timing ──────────────────────────────────────────────
        entry_check = {"action": "NEUTRAL", "reason": "15m data unavailable", "rsi_15m": 50.0, "pullback_target": None}
        ...
        grade_result = None
        try:
            grade_result = grade_signal(df_1h, df_4h, direction, 60.0, rsi_1h_v, 0.0)
        ...
        # ── IMPROVEMENT 3: Tighter SL when accumulation detected ─────────
        sl_price = None
        try:
            if sk_result["is_shakeout"] and sk_result["invalidation"] > 0 and direction == "LONG":
                # Shakeout detected — use flush low as hard SL (tightest possible)
                sl_price = round(sk_result["invalidation"], 6)
            elif acc_result["is_accumulating"] and acc_result["confidence"] == "HIGH" and direction == "LONG":
                # HIGH confidence accumulation — use recent 1H swing low (tighter than 4H structure)
                swing_low = float(df_1h["low"].tail(20).min())
                sl_price  = round(swing_low * 0.988, 6)  # 1.2% below 1H swing low
            elif acc_result["is_accumulating"] and acc_result["confidence"] == "HIGH" and direction == "SHORT":
                swing_high = float(df_1h["high"].tail(20).max())
                sl_price   = round(swing_high * 1.012, 6)
            elif direction == "LONG":
        ...
        # ── Format message — plain text to avoid Markdown entity crashes ──
        def _clean_r(s):
        ...
        msg  = f"🔍 Entry Refinement — {raw_symbol} {direction}{exchange_note}\n"
        msg += f"Your entry: ${entry_price} | Now: ${price_now:.4f} ({price_diff_pct:+.1f}%)\n\n"
        ...
        # ── Entry timing ──────────────────────────────────────────────────
        msg += f"{action_icon} Entry: {action}\n"
        msg += _clean_r(entry_check['reason']) + "\n"
        if entry_check.get("pullback_target"):
            msg += f"Pullback target: ${entry_check['pullback_target']:.4f}\n"

        # ── Multi-TF RSI ──────────────────────────────────────────────────
        msg += f"\nRSI: 15m {rsi_15m_v} / 1H {rsi_1h_v} / 4H {rsi_4h_v}\n"
        if rsi_warning:
            msg += f"⚠ RSI overextended on higher TF — late entry risk\n"
        elif rsi_aligned:
            msg += f"✔ RSI aligned across timeframes\n"

        # ── Grade ─────────────────────────────────────────────────────────
        msg += "\n"
        if grade_result:
            grade_icon = "🥇" if grade_result["grade"] == "A" else ("🥈" if grade_result["grade"] == "B" else "🥉")
            msg += f"{grade_icon} Grade {grade_result['grade']} | Dow: {_clean_r(grade_result.get('dow_phase','UNCLEAR'))}\n"
            if grade_result.get("reasons"):
                msg += "✔ " + _clean_r(" | ".join(grade_result["reasons"][:3])) + "\n"
            if grade_result.get("warnings"):
                msg += "⚠ " + _clean_r(" | ".join(grade_result["warnings"][:2])) + "\n"

        # ── Accumulation ──────────────────────────────────────────────────
        if acc_result["is_accumulating"]:
            msg += f"\n📦 Accumulation ({_clean_r(acc_result['confidence'])}, {acc_result['score']}/7)\n"
            if acc_result["signals"]:
                msg += _clean_r(acc_result['signals'][0]) + "\n"

        # ── Shakeout ──────────────────────────────────────────────────────
        if sk_result["is_shakeout"]:
            msg += f"\n⚡ Shakeout ({_clean_r(sk_result['confidence'])}, {sk_result['score']}/8)\n"
            msg += f"Flush: ${sk_result['flush_low']:.4f} | Target: ${sk_result['pump_target']:.4f}\n"
            if sk_result["signals"]:
                msg += _clean_r(sk_result['signals'][0]) + "\n"

        # ── SL / Liquidation ──────────────────────────────────────────────
        if sl_price:
            sl_pct  = abs(sl_price - entry_price) / entry_price * 100
            liq_est = round(entry_price * (1 - 0.9/leverage), 4) if direction == "LONG" else round(entry_price * (1 + 0.9/leverage), 4)
            # Label SL source so user knows how tight/wide it is
            if sk_result["is_shakeout"] and sk_result["invalidation"] > 0:
                sl_label = "SL (flush low)"
            elif acc_result["is_accumulating"] and acc_result["confidence"] == "HIGH":
                sl_label = "SL (1H swing low — tight)"
            else:
                sl_label = "SL (4H structure)"
            msg += f"\n📌 {sl_label}: ${sl_price} ({sl_pct:.1f}% away)\n"
            msg += f"💥 Liquidation at {leverage}x: ${liq_est}\n"

        # ── Closing verdict ───────────────────────────────────────────────
        if action == "WAIT" and entry_check.get("pullback_target"):
            msg += f"\nWait for pullback to ${entry_check['pullback_target']:.4f} before entering."
        elif action == "ENTER":
            msg += "\nEntry timing looks clean — conditions support entering now."
        else:
            msg += "\nNo strong 15m timing signal — use your own discretion."

        await update.message.reply_text(msg)  # plain text — never crashes

    except Exception as e:
        log.error(f"cmd_refine error: {e}", exc_info=True)
        await update.message.reply_text(f"⚠️ Refinement error: {str(e)[:200]}")
...
async def cmd_paper_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            ...
            grade_result = grade_signal(df_1h, df_4h, direction, 60.0, rsi_v, 0.0)
        ...
        msg  = f"📝 Paper Trade #{trade_id} Opened{_exch_note_p}\n"
        ...
        try:
            ...
            for r in actionable:
                ...
                if is_blacklisted(r["symbol"]):
                    ...
                    continue
                # 2-candle confirmation — only in full pipeline mode
                if _btc_dom_state["mode"] == "full":
                    _df_c = r.get("df_4h")
        ...
        try:
            ...
            for g in gainers:
                ...
                try:
                    ...
                    df_g = r.get("df_4h")
                    if df_g is not None:
                        gr = grade_signal(df_g, r.get("df_4h") or df_g,
                ...
                # ── Path 3: Shakeout → pre-pump LONG alert ────────────────
                    ...
                    try:
                        ...
                        df_1h_g  = r.get("df_1h") or fetch_ohlcv(symbol, "1h", 200)
                        df_4h_g  = r.get("df_4h")
...
async def cmd_watchstatus(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if entry and entry > 0:
        ...
        if cur_price and cur_price > 0:
            ...
            msg += "_Price unavailable — fetching from MEXC..._\n\n"
    ...
    while True:
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                # ── Skip if not on Bitget AND no mexc flag set ────────────────
                if not w.get("use_mexc"):
                    try:
                        exchange.market(symbol)
                    except Exception:
                        log.warning("Watch skip — " + label + " not on Bitget. Use 'mexc' flag: /watch " + label + " ... mexc")
                        continue
