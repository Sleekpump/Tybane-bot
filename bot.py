...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # Condition 3: all 3 groups are flat (none voted for direction)
        _all_flat = all(abs(_gs_now.get(g, 0)) <= 0.12
                        for g in ("trend", "momentum", "structure"))
        if _gs_now and _all_flat:
        ...
        # SKIP fires on 2+ conditions (1 alone could be noise; 2 is a real problem)
        _is_skip = len(_skip_reasons) >= 2
