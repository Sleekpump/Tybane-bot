...
def format_signal(r, sl, tp1, tp2, ob_bias, ob_ratio, pos_usdt, contracts):
    ...
    # Derivatives context — shown when non-neutral
    _dctx = r.get("deriv_ctx", "NEUTRAL")
    _dreason = r.get("deriv_reason", "")
    if _dctx != "NEUTRAL":
        _dctx_emoji = {"ORGANIC": "\U0001f7e2", "SQUEEZE": "\U0001f534", "CROWDED": "\U0001f7e0"}.get(_dctx, "\U0001f7e1")
    ...
    while True:
        ...
        try:
            ...
            for r in results:
                ...
                # Full mode (altseason): Grade A + HIGH + score>=70
                # Simplified mode (BTC season): Grade A still required for alpha
                # Alpha tokens stay at Grade A always — too volatile for Grade B
                _alpha_min_grade = "A"  # alpha always Grade A regardless of mode
                if (direction == "LONG"
        ...
        try:
            ...
            for g in gainers:
                ...
                # ── Path 2: Early breakout → LONG alert ──────────────────────
                      ...
                      # Full mode: Grade A only. Simplified: Grade B allowed for gainers
                      and (r.get("grade") == "A" or (
                           _btc_dom_state["mode"] == "simplified"
                           and r.get("grade") == "B"
                           and r.get("abs_score", 0) >= 75))  # higher score req for B
                ...
                if entry and entry > 0:
                    try:
                        ...
                        if gain_pct >= step_pct:
                            # Try Supertrend line as SL — only when candle data available
                            use_st  = False
                            st_line = 0.0
                            try:
...
def main():
    ...
    async def post_init(application):
        ...
        asyncio.create_task(auto_dominance_monitor(application))
