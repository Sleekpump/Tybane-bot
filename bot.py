...
async def cmd_coin(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        sl, tp1, tp2, _ = calc_levels_v2(r["direction"], r["price"], r["atr"], atr_pct, r.get("df_4h"))
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/coupon \u2014 Best signal on your 50 USDT voucher pairs\n"
...
async def cmd_coupon(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """
    /coupon — Scans the 7 Bitget coupon-eligible pairs and returns the best signal.
    50 USDT position boost voucher valid until 2026-05-07.
    Valid pairs: PIUSDT, DOGEUSDT, BGBUSDT, XRPUSDT, SUIUSDT, PEPEUSDT, SHIBUSDT
    """
    COUPON_PAIRS = [
        "PI/USDT:USDT",
        "DOGE/USDT:USDT",
    ...
    await update.message.reply_text(
        "\U0001f3ab *Coupon Scanner*\n\n"
        "Scanning 7 coupon-eligible pairs (50 USDT boost)...\n"
    ...
    try:
        ...
        for symbol in COUPON_PAIRS:
            try:
                ...
                try:
                    ...
                    if df_g is not None:
                        gr = grade_signal(df_g, df_4g, r["direction"], r.get("abs_score", 0), r.get("rsi_4h", 50), r.get("funding", 0))
                        r["grade"]          = gr["grade"]
                ...
                # Only keep MOMENTUM signals — exclude WEAK and Grade C
                if r.get("signal_type") != "MOMENTUM":
                    continue
                if r.get("grade") == "C":
                    continue

                # Block LONGs if circuit breaker active
                if r["direction"] == "LONG" and btc_circuit_breaker["active"]:
                    continue

                results.append(r)
                log.info("Coupon scan: " + COUPON_LABELS[symbol] + " " + r["direction"] + " Q:" + str(r.get("abs_score", 0)))

            except Exception as e:
                log.warning("Coupon scan error for " + symbol + ": " + str(e))

        if not results:
            await update.message.reply_text(
                "\U0001f3ab *Coupon Scanner*\n\n"
                "No MOMENTUM signals on the 7 coupon pairs right now.\n"
                "Try again in 15-30 minutes.",
                parse_mode="Markdown"
            )
            return

        # Pick best — Grade A first, then highest quality score
        grade_order = {"A": 0, "B": 1, "C": 2}
        ...
        sl, tp1, tp2, vp_meta = calc_levels_v2(best["direction"], best["price"], best["atr"], atr_pct, best.get("df_4h"))
        ...
        # Header with coupon context
        header  = "\U0001f3ab *Coupon Signal: " + best.get("label", best["symbol"].split("/")[0]) + "*\n"
        header += "50 USDT position boost active \u2014 use the voucher on this trade\n\n"

        # Show all scanned results as summary
        summary = "\U0001f4cb *All 7 pairs scanned:*\n"
        pair_labels = {s: COUPON_LABELS[s] for s in COUPON_PAIRS}
        scanned_symbols = [r["symbol"] for r in results]
        for sym, lbl in pair_labels.items():
            if sym in scanned_symbols:
                ...
                summary += ge + " *" + lbl + "* " + match["direction"] + " | Q:" + str(match.get("abs_score", 0)) + " | " + g + "\n"
            else:
...
async def cmd_watching(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol, w in watched_trades.items():
        ...
        msg += "• *" + w["label"] + "* " + direction + " " + type_label + " | Entry: " + entry_str + " | 2/4 signals trigger"
        if price:
...
async def _check_early_warning(df: "pd.DataFrame", direction: str) -> tuple:
    """
    4 early warning checks for reversal against the given trade direction.
    Returns (signals_triggered: list[str], count: int)

    Checks (on whichever TF df is passed — 1H for scalp, 4H for swing):
      1. RSI divergence  — price lower low but RSI higher low (bullish) or vice versa
      2. Volume exhaustion — big dump/pump candle followed by volume collapse
      3. MACD crossover  — DIF crosses DEA in reversal direction
      4. ROC weakening   — momentum flattening/turning in reversal direction
    """
    import pandas_ta as _ta
    signals = []
    ...
    try:
        ...
        if roc is not None and len(roc.dropna()) >= 5:
            ...
            if direction == "SHORT":
                # Was negative (downtrend momentum), now flattening or positive
                was_negative = roc_v[-3] < -1.0
                now_weakening = roc_v[-1] > roc_v[-3]  # ROC rising = momentum weakening
                if was_negative and now_weakening and roc_v[-1] > -0.5:
                    signals.append("Downward momentum weakening (ROC)")
            else:
                # Was positive (uptrend momentum), now flattening or negative
...
async def auto_watch(app):
    ...
    Uses 4 early warning signals — alert fires when 2+ agree.
    Two-level early warning system:

    SWING trades:
      Level 1 — 1H early warning  (⚡): 2/4 signals on 1H → tighten SL, prepare to exit
      Level 2 — 4H confirmed (⚠):  2/4 signals on 4H → reversal confirmed, close trade
      Level 1 cooldown: 60 min | Level 2 cooldown: 30 min
      Level 2 always checked first — if it fires, Level 1 is skipped that cycle

    SCALP trades:
      Single level on 15m — 2/4 signals → close trade
      Cooldown: 30 min

    4 checks: RSI divergence, Volume exhaustion, MACD crossover, ROC weakening.
    """
            ...
            try:
                ...
                if l2_cooldown_ok:
                    ...
                    if count_4h >= 2:
                        ...
                        msg += "Current price: `$" + "{:.4f}".format(price) + "`\n"
                        msg += pnl_str
                        msg += "\n4H signals (" + str(count_4h) + "/4 agree):\n" + sigs_str + "\n"
                ...
                if l1_cooldown_ok:
                    ...
                    if count_1h >= 2:
                        ...
                        msg += "Current price: `$" + "{:.4f}".format(price) + "`\n"
                        msg += pnl_str
                        msg += "\n1H signals (" + str(count_1h) + "/4 agree):\n" + sigs_str + "\n"
