...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Funding rate ──────────────────────────────────────────────────────
        # Extreme positive funding on LONG = perp traders heavily long (fade risk)
        # Extreme negative funding on SHORT = perp traders heavily short (squeeze risk)
        funding_warn  = ""
        funding_rate  = 0.0
        try:
            _fr = exchange.fetch_funding_rate(symbol)
            funding_rate = float(_fr.get("fundingRate", 0.0)) * 100  # as %
            if direction == "LONG" and funding_rate > 0.1:
                funding_warn = (f"⚠ Funding: +{funding_rate:.3f}% — perp traders heavily long, "
                                f"smart money may fade this move")
            elif direction == "SHORT" and funding_rate < -0.1:
                funding_warn = (f"⚠ Funding: {funding_rate:.3f}% — perp traders heavily short, "
                                f"squeeze risk if price pushes up")
            elif abs(funding_rate) > 0.05:
                funding_warn = f"Funding: {funding_rate:+.3f}% (elevated)"
        except Exception:
        ...
        # ── Context signals (VWAP / Funding / BTC) ───────────────────────────
        # These are probability modifiers — they don't change the verdict
        # but tell you how much additional risk exists.
        context_signals = []

        # VWAP distance — already computed inside grade_result
        if grade_result:
            _vwap_dist = grade_result.get("vwap_dist_pct", 0)
            _vwap_bias = grade_result.get("vwap_bias", "AT")
            if _vwap_dist and abs(_vwap_dist) > 5.0:
                ...
                if direction == "LONG" and _vwap_dist > 10.0:
                    context_signals.append(
                        f"⚠ VWAP: Price {abs(_vwap_dist):.1f}% above VWAP — "
                        f"premium to fair value, higher reversion risk"
                    )
                elif direction == "SHORT" and _vwap_dist < -10.0:
                    context_signals.append(
                        f"⚠ VWAP: Price {abs(_vwap_dist):.1f}% below VWAP — "
                        f"discount to fair value, bounce risk"
                    )
                else:
                    context_signals.append(
                        f"VWAP: Price {abs(_vwap_dist):.1f}% {_vwap_side} "
                        f"${grade_result.get('vwap', 0):.4f}"
                    )

        if funding_warn:
            context_signals.append(funding_warn)

        if btc_context:
            context_signals.append(btc_context)
        ...
        # OB zone — labelled clearly so user knows which end is entry vs invalidation
        if _ob_ref:
        ...
        # Watch command — use pullback entry if waiting
        _watch_entry = entry_check.get("pullback_target") or _ref
        _watch_tp    = f" {tp1_pct_r} {tp2_pct_r}" if (tp1_pct_r and tp2_pct_r) else ""
        _watch_type  = trade_type
        msg += f"\n`/watch {raw_symbol} {direction} {_watch_entry} {_watch_type} {leverage}{_watch_tp}`"

        # ── Counter-trade suggestion ──────────────────────────────────────────
        # When waiting for a pullback to the OB for a LONG, the move from current
        # price DOWN to the OB is itself a tradeable short scalp — enter short now,
        # TP at the OB top (which flips to the long entry), then flip direction.
        # Only show when: WAIT verdict + OB exists + distance >= 5% (meaningful move).
        ...
        if (action in ("WAIT", "CONDITIONAL")
            ...
            if _counter_dist_pct >= 5.0:
                ...
                # Counter SL: above recent swing high (SHORT) or below swing low (LONG)
                # Use 1H last 5 bars — tight because this is a scalp
                try:
                    ...
                    # Counter TP = pullback target (OB top for LONG setup)
                    _c_tp      = round(_pb_target, 4)
                    _c_tp_pct  = round(_counter_dist_pct, 1)

                    # Only show if RR is at least 1.5:1
                    _c_rr = _c_tp_pct / _c_sl_pct if _c_sl_pct > 0 else 0

                    if _c_rr >= 1.5:
                        msg += "\n\n---"
                        msg += "\n⚡ *While you wait - Counter Scalp*"
                        msg += (f"\nPullback to long entry is "
                                f"{_counter_dist_pct:.1f}% - tradeable as a "
                                f"{'short' if _counter_dir == 'SHORT' else 'long'} scalp first.\n")
                        msg += f"\n*{_counter_dir} scalp levels:*\n"
                        msg += f"Entry: ${price_now:.4f} (now)\n"
                        msg += f"TP: ${_c_tp} ({_c_tp_pct:.1f}% - OB top = your long entry)\n"
                        msg += f"SL: ${_c_sl} ({_c_sl_pct:.1f}%)\n"
                        msg += f"RR: {_c_rr:.1f}:1\n"
                        msg += (f"\nWhen TP hits - flip to LONG at ${_c_tp} "
                                f"with SL below OB at ${_ob_ref['low']:.4f}\n")
                        msg += f"`/refine {raw_symbol} {_counter_dir} {price_now:.4f} {leverage} scalp`"
                except Exception:
                    pass  # skip counter-trade if levels can't be calculated
