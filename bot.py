    ...
    try:
        ...
        try:
            ...
            if adx_df is not None and not adx_df.empty:
                adx_val = float(adx_df.iloc[-1, 0])  # ADX value
                dmp_val = float(adx_df.iloc[-1, 1])  # DI+
                dmn_val = float(adx_df.iloc[-1, 2])  # DI-
                if adx_val > 28:
        ...
        # TP note — direct to /watch for dynamic TP tracking
        msg += "\n💡 For TP1/TP2 alerts: /watch " + raw_symbol + " " + direction + " " + str(entry_price) + " swing high " + str(leverage)
