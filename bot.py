...
def format_signal(r, sl, tp1, tp2, ob_bias, ob_ratio, pos_usdt, contracts):
    ...
    if r.get("shakeout_signal") or r.get("shakeout"):
        ...
        shakeout_tag = f"\n⚡ *SHAKEOUT* ({sk_conf}, {sk_sc}/8) — flush `${sk_low:.4f}` reclaimed\n"
...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        grade_result = None
        try:
            rsi_1h_s = __import__("pandas_ta").rsi(df_1h["close"], length=14)
            rsi_1h_v = float(rsi_1h_s.dropna().iloc[-1]) if rsi_1h_s is not None and len(rsi_1h_s.dropna()) > 0 else 50.0
        ...
        try:
            if sk_result["is_shakeout"] and sk_result["invalidation"] > 0 and direction == "LONG":
                sl_price = round(sk_result["invalidation"], 6)  # use flush low as hard SL
            elif direction == "LONG":
        ...
        if grade_result:
            ...
            msg += f"{grade_icon} *Grade {grade_result['grade']}* | Dow: {grade_result.get('dow_phase', 'UNCLEAR')}\n"
            if grade_result.get("reasons"):
                msg += "✔ " + " | ".join(grade_result["reasons"][:3]) + "\n"
            if grade_result.get("warnings"):
                msg += "⚠ " + " | ".join(grade_result["warnings"][:2]) + "\n"

        if acc_result["is_accumulating"]:
            msg += f"\n📦 *Accumulation detected* ({acc_result['confidence']}, score {acc_result['score']}/7)\n"
            if acc_result["signals"]:
                msg += f"_{acc_result['signals'][0]}_\n"

        if sk_result["is_shakeout"]:
            msg += f"\n⚡ *Shakeout detected* ({sk_result['confidence']}, score {sk_result['score']}/8)\n"
            msg += f"Flush low: `${sk_result['flush_low']:.4f}` | Pump target: `${sk_result['pump_target']:.4f}`\n"
            if sk_result["signals"]:
                msg += f"_{sk_result['signals'][0]}_\n"
            msg += "_This looks like a manipulated flush — crimepump may follow._\n"

        if sl_price:
            sl_pct = abs(sl_price - entry_price) / entry_price * 100
            if direction == "LONG":
                liq_est = round(entry_price * (1 - 0.9 / leverage), 4)
            else:
                liq_est = round(entry_price * (1 + 0.9 / leverage), 4)
            msg += f"\n📌 *Suggested SL:* `${sl_price}` ({sl_pct:.1f}% away)\n"
            msg += f"💥 *Est. liquidation* at {leverage}x: `${liq_est}`\n"

        if action == "WAIT" and entry_check.get("pullback_target"):
            msg += f"\n_Wait for pullback to `${entry_check['pullback_target']:.4f}` before entering._"
        elif action == "ENTER":
            msg += "\n_Entry timing looks clean — conditions support entering now._"
        else:
            msg += "\n_No strong 15m timing signal — use your own discretion._"

        await send_msg(update.get_bot() or ctx.bot, msg, chat_id=update.effective_chat.id)
...
async def cmd_paper_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if df_1h is None or df_4h is None:
            await update.message.reply_text("⚠️ Could not fetch data. Check ticker.")
            return
        ...
        msg  = f"📝 *Paper Trade #{trade_id} Opened*\n"
        msg += f"*{raw_symbol} {direction}* @ `${entry_price}` | {leverage}x\n\n"
        msg += f"{action_icon} *15m Entry: {action}*\n_{entry_check['reason']}_\n"
        if entry_check.get("pullback_target"):
            msg += f"Pullback target: `${entry_check['pullback_target']:.4f}`\n"
        msg += f"\n{grade_icon} *Grade {grade_str}* | Dow: {dow_phase}\n"
        if grade_result and grade_result.get("reasons"):
            msg += "✔ " + " | ".join(grade_result["reasons"][:2]) + "\n"
        if grade_result and grade_result.get("warnings"):
            msg += "⚠ " + " | ".join(grade_result["warnings"][:1]) + "\n"
        if sk["is_shakeout"]:
            msg += f"\n⚡ *Shakeout detected* ({sk['confidence']}, {sk['score']}/8)\n"
            msg += f"Flush: `${sk['flush_low']:.4f}` | Target: `${sk['pump_target']:.4f}`\n"
        if acc["is_accumulating"]:
            msg += f"\n📦 *Accumulation* ({acc['confidence']}, {acc['score']}/7)\n"
        msg += f"\n📌 SL: `${sl:.4f}` ({sl_pct:.1f}%) | TP1: `${tp1:.4f}` ({tp1_pct:.1f}%) | TP2: `${tp2:.4f}` ({tp2_pct:.1f}%)\n"
        msg += f"💥 Liquidation at {leverage}x: `${liq:.4f}`\n"
        msg += f"\n_Track with /portfolio — close analysis with /paper_stats_"

        await send_msg(ctx.bot, msg, chat_id=update.effective_chat.id)
...
async def cmd_watch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    lev_note   = "\n\u26a1 Leverage: *" + str(watch_lev) + "x*" if watch_lev != LEVERAGE else ""
                ...
                if entry and entry > 0:
                    try:
                        ...
                        if gain_pct >= step_pct:
                            ...
                            if gain_pct < 10:
                                lock_pct = gain_pct * 0.60
                            elif gain_pct < 25:
                                lock_pct = gain_pct * 0.40
                            else:
                                lock_pct = gain_pct * 0.25

                            if zlema_still_bullish and gain_pct >= 15:
                                lock_pct *= 0.85  # trend intact — give 15% more room
