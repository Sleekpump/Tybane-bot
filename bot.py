...
async def cmd_refine(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        # ── Price gap warning ─────────────────────────────────────────────
        # If price has already moved significantly from the intended entry,
        # the timing analysis below reflects current price, not the intended entry.
        # This warns the user so they don't misread ENTER as "your original entry is still valid."
        gap_warning = ""
        abs_gap = abs(price_diff_pct)
        if abs_gap >= 5.0:
            moved = "above" if price_diff_pct > 0 else "below"
            if direction == "LONG" and price_diff_pct > 5.0:
                gap_warning = ("⚠ Price already moved +" + "{:.1f}".format(abs_gap)
                               + "% above your entry. Entry timing below reflects NOW, not $"
                               + "{:.4f}".format(entry_price) + ". Consider waiting for a pullback.")
            elif direction == "LONG" and price_diff_pct < -5.0:
                gap_warning = ("⚠ Price is -" + "{:.1f}".format(abs_gap)
                               + "% below your intended entry. You may get a better fill now than $"
                               + "{:.4f}".format(entry_price) + ".")
            elif direction == "SHORT" and price_diff_pct < -5.0:
                gap_warning = ("⚠ Price already dropped -" + "{:.1f}".format(abs_gap)
                               + "% from your entry. Timing below reflects NOW. Chasing a dump is risky.")
            elif direction == "SHORT" and price_diff_pct > 5.0:
                gap_warning = ("⚠ Price is +" + "{:.1f}".format(abs_gap)
                               + "% above your intended SHORT entry $" + "{:.4f}".format(entry_price)
                               + ". May be getting a better entry zone now.")
        ...
        if df_15m is not None and len(df_15m) >= 20:
            ...
            try:
                from signal_engine import detect_order_blocks
        ...
        # Apply strong trend override: if trend is strong, demote WAIT to NEUTRAL
        # so the user isn't told to wait for a bounce that may never happen
        if strong_trend_override and entry_check["action"] == "WAIT":
        ...
        # ── SL — tighter when accumulation HIGH or shakeout detected ─────
        sl_price = None
        try:
            if sk_result["is_shakeout"] and sk_result["invalidation"] > 0 and direction == "LONG":
                ...
                sl_price  = round(swing_low * 0.988, 6)
        ...
        # ── TP1 / TP2 from structure ──────────────────────────────────────
        # Previously missing entirely — user had no target, panicked and left early.
        # Use same calc_levels_v2 that paper_refine and auto_scan use.
        tp1_price = tp2_price = None
        tp1_pct_r = tp2_pct_r = None
        try:
            from risk_manager import calc_levels_v2
            _atr_r  = float(ta.atr(df_1h["high"], df_1h["low"], df_1h["close"], length=14).dropna().iloc[-1])
            _ap_r   = (_atr_r / entry_price * 100) if entry_price > 0 else 1.5
            _, tp1_price, tp2_price, _ = calc_levels_v2(direction, entry_price, _atr_r, _ap_r, df_4h)
            tp1_price = round(tp1_price, 6)
            tp2_price = round(tp2_price, 6)
            tp1_pct_r = round(abs(tp1_price - entry_price) / entry_price * 100, 1)
            tp2_pct_r = round(abs(tp2_price - entry_price) / entry_price * 100, 1)
        ...
        msg = "🔍 Entry Refinement — " + raw_symbol + " " + direction + exchange_note + "\n"
        msg += "Your entry: $" + str(entry_price) + " | Now: $" + "{:.4f}".format(price_now)
        msg += " (" + "{:+.1f}".format(price_diff_pct) + "%)\n"

        # Price gap warning — shown immediately if price has moved significantly
        if gap_warning:
            msg += "\n" + gap_warning + "\n"

        msg += "\n" + action_icon + " Entry: " + action + "\n"
        msg += _c(entry_check["reason"]) + "\n"
        if entry_check.get("pullback_target"):
            msg += "Pullback target: $" + "{:.4f}".format(entry_check["pullback_target"]) + "\n"
        if strong_trend_override and action == "ENTER":
            msg += trend_strength_note + "\n"

        # Grade
        msg += "\n"
        if grade_result:
            grade_icon = "🥇" if grade_result["grade"] == "A" else ("🥈" if grade_result["grade"] == "B" else "🥉")
            msg += grade_icon + " Grade " + grade_result["grade"] + " | Dow: " + _c(grade_result.get("dow_phase","UNCLEAR")) + "\n"
            if grade_result.get("reasons"):
                msg += "✔ " + _c(" | ".join(grade_result["reasons"][:3])) + "\n"
            if grade_result.get("warnings"):
                msg += "⚠ " + _c(" | ".join(grade_result["warnings"][:2])) + "\n"

        # Accumulation
        if acc_result["is_accumulating"]:
            msg += "\n📦 Accumulation (" + _c(acc_result["confidence"]) + ", " + str(acc_result["score"]) + "/7)\n"
            if acc_result["signals"]:
                msg += _c(acc_result["signals"][0]) + "\n"

        # Shakeout
        if sk_result["is_shakeout"]:
            msg += "\n⚡ Shakeout (" + _c(sk_result["confidence"]) + ", " + str(sk_result["score"]) + "/8)\n"
            msg += "Flush: $" + "{:.4f}".format(sk_result["flush_low"]) + " | Target: $" + "{:.4f}".format(sk_result["pump_target"]) + "\n"
            if sk_result["signals"]:
                msg += _c(sk_result["signals"][0]) + "\n"

        # FVG levels
        if fvg_levels:
            msg += "\n📐 Fair Value Gaps (" + ("support zones" if direction == "LONG" else "resistance zones") + "):\n"
            for fvg in fvg_levels:
                dist = abs(fvg["mid"] - price_now) / price_now * 100
                msg += ("  " + fvg["tf"] + " " + fvg["type"] + " FVG: $"
                        + "{:.4f}".format(fvg["bot"]) + " - $" + "{:.4f}".format(fvg["top"])
                        + " (mid $" + "{:.4f}".format(fvg["mid"]) + ", "
                        + "{:.1f}".format(dist) + "% away)\n")
            if direction == "LONG":
                msg += "Re-entry zone if price pulls back to these levels.\n"
            else:
                msg += "Entry/add zone if price bounces to these levels.\n"
        ...
        if sl_price:
            ...
            msg += "\n📌 " + sl_label + ": $" + str(sl_price) + " (" + "{:.1f}".format(sl_pct) + "% away)\n"
            msg += "💥 Liquidation at " + str(leverage) + "x: $" + str(liq_est) + "\n"

        # TP levels — shown directly so you have targets before entering
        if tp1_price and tp2_price:
            msg += "\n🎯 TP1: $" + "{:.6f}".format(tp1_price) + " (+" + str(tp1_pct_r) + "%"
            msg += " / +" + "{:.1f}".format(tp1_pct_r * leverage) + "% at " + str(leverage) + "x)\n"
            msg += "🎯 TP2: $" + "{:.6f}".format(tp2_price) + " (+" + str(tp2_pct_r) + "%"
            msg += " / +" + "{:.1f}".format(tp2_pct_r * leverage) + "% at " + str(leverage) + "x)\n"

        # Watch suggestion — pre-filled with TP% so alerts actually fire
        _watch_tp = ""
        if tp1_pct_r and tp2_pct_r:
            _watch_tp = " " + str(tp1_pct_r) + " " + str(tp2_pct_r)
        msg += "\n💡 /watch " + raw_symbol + " " + direction + " " + str(entry_price) + " swing high " + str(leverage) + _watch_tp

        # Closing verdict
        if action == "WAIT" and entry_check.get("pullback_target"):
            msg += "\n\nWait for pullback to $" + "{:.4f}".format(entry_check["pullback_target"]) + " before entering."
        elif action == "ENTER":
    ...
    try:
        if w.get("use_mexc"):
            ...
            try:
                ...
                # Primary exchange failed — silently try any available exchange
    ...
    for symbol, w in watched_trades.items():
        ...
        try:
            _raw_price = (fetch_price_any_exchange(symbol)
                          if w.get("use_mexc")
                          else exchange.fetch_ticker(symbol)["last"])
