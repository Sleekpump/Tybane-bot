...
WATCH_QUALITY_MIN   = 55  # minimum reversal quality to alert
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/watch BTC SHORT 95000 \u2014 Monitor trade for reversals\n"
...
async def cmd_watch(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if len(args) >= 3:
        try:
            ...
            await update.message.reply_text("Invalid entry price. Example: /watch BTC SHORT 95000")
            return
    ...
    await update.message.reply_text(
        "👀 *Now watching " + coin + "*\n\n"
        "Direction: *" + direction + "*" + entry_str + "\n"
        "Checking every 2 minutes.\n"
        "Alert threshold: quality ≥ " + str(WATCH_QUALITY_MIN) + " or regime flip.\n\n"
...
async def cmd_watching(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    for symbol, w in watched_trades.items():
        ...
        msg += "• *" + w["label"] + "* " + direction + " | Entry: " + entry_str
        if price:
...
async def auto_watch(app):
    ...
    Alerts when reversal quality >= 55 or regime flips against trade direction.
    """
    ...
    while True:
        ...
        for symbol, w in list(watched_trades.items()):
            try:
                ...
                from signal_engine import analyze_v2, detect_regime
                df_1h = fetch_ohlcv(symbol, "1h", 200)
                df_4h = fetch_ohlcv(symbol, "4h", 100)
                price = float(df_1h["close"].iloc[-1])

                # Regime check — instant alert if regime flips against direction
                ...
                # Quality check — run full Phase 1
                ...
                reversal_detected = (
                    ...
                    quality_score >= WATCH_QUALITY_MIN
                )
                ...
                if regime_flip:
                    ...
                    trigger_str = "Reversal quality: `" + str(round(quality_score, 1)) + "/100` | " + confidence
