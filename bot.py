...
def classify_signal(df, direction, score, btc_change_24h=None):
    ...
    try:
        ...
        # ── 1b. SHORT: sustained downtrend structure (fixes SHORT bias) ─────────
        # Breakdown check requires a new low every scan — unrealistic in sustained downtrend.
        # Add alternative: price consistently below resistance and EMA stack bearish.
        if direction == "SHORT" and momentum_pts == 0:
            import pandas_ta as _pta
            ema9_s  = _pta.ema(pd.Series(close), length=9)
            ema21_s = _pta.ema(pd.Series(close), length=21)
            ema50_s = _pta.ema(pd.Series(close), length=50)
            if ema9_s is not None and ema21_s is not None and ema50_s is not None:
                e9  = float(ema9_s.dropna().iloc[-1])  if len(ema9_s.dropna())  > 0 else close[-1]
                e21 = float(ema21_s.dropna().iloc[-1]) if len(ema21_s.dropna()) > 0 else close[-1]
                e50 = float(ema50_s.dropna().iloc[-1]) if len(ema50_s.dropna()) > 0 else close[-1]
                # Bearish EMA stack: 9 < 21 < 50 AND price below all
                bearish_stack = e9 < e21 < e50 and close[-1] < e9
                if bearish_stack:
                    momentum_pts += 2
                    reasons.append("bearish EMA stack (9<21<50) — sustained downtrend")
            # Also: price below 20-candle resistance with 3+ consecutive red candles
            red_candles = sum(1 for i in range(1, 5) if close[-i] < close[-i-1])
            if red_candles >= 3 and close[-1] < resistance_20:
                momentum_pts += 1
                reasons.append(str(red_candles) + " consecutive red candles below resistance")
        ...
        if btc_change_24h is not None:
            ...
            # Lower threshold for SHORT — even -1.5% relative weakness counts
            elif direction == "SHORT" and -3 <= relative_strength < -1.5:
                momentum_pts += 1
                reasons.append("underperforming BTC by " + str(round(relative_strength, 1)) + "% (weak)")
        ...
        if direction == "LONG":
            ...
            # SHORT: sustained negative ROC even without volume spike counts
            roc_ok = roc < -1.0
        if roc_ok and vol_increasing:
            momentum_pts += 1
            reasons.append("accelerating with volume")
        elif direction == "SHORT" and roc < -0.5:
            # SHORT: even mild negative ROC with any volume is signal in downtrend
            momentum_pts += 1
            reasons.append("negative momentum ROC (" + str(round(roc, 1)) + "%)")
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/flip COIN LONG 5 \u2014 Hunt flip scalp entries during swing\n"
    ...
    while True:
        ...
        try:
            ...
            for r in results:
                ...
                if exh.get("short_opp") and now - alpha_state["last_signals"].get(symbol, 0) >= ALPHA_COOLDOWN:
                    from risk_manager import calc_levels_v2
                    _ap = (r["atr"] / r["price"] * 100) if r.get("price", 0) > 0 else 1.0
                    ...
                    header += "Blow-off top detected — consider SHORT\n"
...
async def cmd_flip(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    flip_trades[symbol] = {
        ...
        "last_flip":   0,        # timestamp of last flip alert
        "flip_count":  0,        # how many flips fired
    ...
    await update.message.reply_text(
        ...
        "\U0001f552 Checking every 2 min on 15m chart\n\n"
        "_Alert fires when 2+ reversal signals agree on 15m._\n"
...
def _check_flip_signals(df_15m: "pd.DataFrame", direction: str) -> tuple:
    """
    Check 15m chart for flip entry signals.
    Returns (signals: list[str], count: int, entry_quality: str)

    4 checks specifically tuned for erratic swing oscillations:
      1. RSI extreme — <32 for LONG flip, >68 for SHORT flip
      2. EMA9 slope reversal — slope changing direction on 15m
      3. Candle structure — 3 candles forming reversal pattern
      4. Volume dry-up — volume declining into the reversal (exhaustion of current move)
    """
    ...
    try:
        ...
        if len(close) >= 5:
            ...
            l = low.values
            if direction == "LONG":
                # 3 down candles then a bullish close above prior candle's open
                three_red = c[-4] < c[-5] and c[-3] < c[-4] and c[-2] < c[-3]
                reversal  = c[-1] > c[-2]  # current candle closing up
                hammer    = (c[-2] - l[-2]) > abs(c[-2] - c[-3]) * 1.5  # hammer wick
                ...
                # 3 up candles then a bearish close below prior candle's open
        ...
        # ── 4. Volume dry-up into reversal ────────────────────────────────────
        # Volume declining over last 3 candles = exhaustion of the current move
        if len(volume) >= 6:
            v = volume.values
            vol_ma    = float(volume.rolling(20).mean().iloc[-1]) if len(volume) >= 20 else float(volume.mean())
            declining = v[-3] > v[-2] > v[-1]       # 3 consecutive declining candles
            low_vol   = v[-1] < vol_ma * 0.7         # current vol below average
            if declining and low_vol:
                signals.append("Volume drying up — current move exhausting")

    except Exception as e:
        log.warning("Flip signal check error: " + str(e))
