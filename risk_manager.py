...
MAX_PORTFOLIO_HEAT  = 4.5     # max total open risk % = 3 trades at 1.5% each (was 6%/4 trades)
MAX_CORRELATED      = 2       # max simultaneous signals in highly correlated group
MAX_SAME_DIRECTION  = 2       # max open trades in same direction (was 3 — tighter to reduce noise)
                              # prevents 6x LONG exposure in a BTC dump
CORRELATION_GROUPS  = {
...
SESSION_DEAD_HOURS  = {1, 2, 3}  # UTC hours — avoid signals (reduced: 01:00-03:00 UTC deepest dead zone)
...
def calc_levels_v2(
    ...
    """
    Volatility-adaptive SL/TP levels with optional volume profile alignment.

    If df (OHLCV DataFrame) is provided, levels are snapped to volume profile
    HVNs and POC for stronger support/resistance. Otherwise uses pure ATR.
    ...
    # Guard against corrupted ATR values — clamp to 0.3%-15% of price
    if atr <= 0 or atr > price * 0.15:
        atr     = price * 0.02   # fallback: 2% of price
        atr_pct = 2.0
        log.warning(f"calc_levels_v2: invalid ATR={atr:.6f} for price={price:.6f} — using 2% fallback")
    ...
    # Guard against NEUTRAL or invalid direction — default to LONG
    if direction not in ("LONG", "SHORT"):
        direction = "LONG"
    # Guard against zero/negative ATR
    if atr <= 0 or atr > price:
        atr = price * (atr_pct / 100) if atr_pct and atr_pct > 0 else price * 0.02

    if direction == "LONG":
        sl_atr  = round(price - atr * sl_mult,  6)
        tp1_atr = round(price + atr * tp1_mult, 6)
        tp2_atr = round(price + atr * tp2_mult, 6)
    else:
        sl_atr  = round(price + atr * sl_mult,  6)
        tp1_atr = round(price - atr * tp1_mult, 6)
        tp2_atr = round(price - atr * tp2_mult, 6)
    ...
    # ── Volume Profile level adjustment ──────────────────────────────────────
    if df is not None:
        try:
            from signal_engine import compute_volume_profile, vp_adjusted_levels
            vp = compute_volume_profile(df)
            sl, tp1, tp2, vp_meta = vp_adjusted_levels(
                direction, price, atr, vp, sl_atr, tp1_atr, tp2_atr
            )
            meta["vp_poc"]         = vp_meta.get("poc")
            meta["vp_hvn_above"]   = vp_meta.get("hvn_above")
            meta["vp_hvn_below"]   = vp_meta.get("hvn_below")
            meta["vp_in_lvn"]      = vp_meta.get("in_lvn", False)
            meta["vp_adjustments"] = vp_meta.get("adjustments", [])
            return sl, tp1, tp2, meta
        except Exception as e:
            log.warning(f"VP level adjustment failed: {e} — using ATR levels")

    return sl_atr, tp1_atr, tp2_atr, meta
...
# ─── 3b. DIRECTION CAP ───────────────────────────────────────────────────────
def check_direction_cap(
    direction: str,
    active_signals: dict,
    max_same: int = MAX_SAME_DIRECTION,
) -> dict:
    """
    Prevents opening too many trades in the same direction.
    Stops scenario where 6x LONG trades all bleed during a BTC dump.
    Returns {"pass": bool, "reason": str, "count": int}
    """
    same_dir = [s for s, v in active_signals.items()
                if v.get("direction", "") == direction]
    count = len(same_dir)

    if count >= max_same:
        coins = ", ".join([s.split("/")[0] for s in same_dir])
        return {
            "pass":   False,
            "reason": f"Too many {direction} trades open ({count}/{max_same}): {coins}",
            "count":  count,
        }
    return {
        "pass":   True,
        "reason": f"{direction} slots: {count}/{max_same} used",
        "count":  count,
    }
...
def risk_gate(
    ...
    dir_cap  = check_direction_cap(direction, active_signals)
...
def format_risk_block(risk_result: dict) -> str:
    ...
    dir_cap   = risk_result.get("checks", {}).get("direction", {})
    ...
    if dir_cap:
        msg += f"↕ Direction slots: `{dir_cap.get('reason', '')}` \n"
